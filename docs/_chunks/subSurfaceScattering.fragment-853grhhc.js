import{se}from"./site-82m1a6bm.js";import{Ml}from"./site-rmstvecm.js";import{i}from"./site-1yf4ncc8.js";var r="fibonacci",a=`#define rcp(x) 1./x
#define GOLDEN_RATIO 1.618033988749895
vec2 Golden2dSeq(int i,float n)
{return vec2(float(i)/n+(0.5/n),fract(float(i)*rcp(GOLDEN_RATIO)));}
vec2 SampleDiskGolden(int i,int sampleCount)
{vec2 f=Golden2dSeq(i,float(sampleCount));return vec2(sqrt(f.x),TWO_PI*f.y);}`;if(!i.IncludesShadersStore[r])i.IncludesShadersStore[r]=a;var n={name:r,shader:a};var t="diffusionProfile",l="uniform vec3 diffusionS[5];uniform float diffusionD[5];uniform float filterRadii[5];";if(!i.IncludesShadersStore[t])i.IncludesShadersStore[t]=l;var c={name:t,shader:l};var o="subSurfaceScatteringPixelShader",f=`#include<helperFunctions>
#include<fibonacci>
...
totalWeight=max(totalWeight,HALF_MIN);gl_FragColor=vec4(inputColor.rgb+albedo*max(totalIrradiance/totalWeight,vec3(0.0)),1.);}`;if(!i.ShadersStore[o])i.ShadersStore[o]=f;var s=[se,n,Ml,c];for(let e of s)if(!i.IncludesShadersStore[e.name])i.IncludesShadersStore[e.name]=e.shader;var g={name:o,shader:f};export{g as subSurfaceScatteringPixelShader};

//# debugId=31AEBF9D7A13C9FD64756E2164756E21
//# sourceMappingURL=subSurfaceScattering.fragment-853grhhc.js.map
