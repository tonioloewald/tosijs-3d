import{_B as e}from"./site-ea0e8ybd.js";var t="ssao2PixelShader",o=`precision highp float;uniform sampler2D textureSampler;varying vec2 vUV;
#ifdef SSAO
...
#endif
uniform mat3 depthProjection;uniform float maxZ;uniform float minZAspect;uniform vec2 texelSize;uniform mat4 projection;
#ifdef NORMAL_WORLDSPACE
uniform mat3 normalWorldToView;
#endif
void main()
{vec3 random=textureLod(randomSampler,vUV*randTextureTiles,0.0).rgb;float depth=textureLod(depthSampler,vUV,0.0).r;float depthSign=sign(depth);depth=depth*depthSign;vec3 normal=normalize(textureLod(normalSampler,vUV,0.0).rgb);
#ifdef NORMAL_WORLDSPACE
normal=normalize(normalWorldToView*normal);
#endif
float occlusion=0.0;float correctedRadius=min(radius,minZAspect*depth/near);
#ifdef ORTHOGRAPHIC_CAMERA
...
#endif
`;if(!e.ShadersStore[t])e.ShadersStore[t]=o;var a={name:t,shader:o};
export{a as ok};

//# debugId=B417FE937A2456AF64756E2164756E21
//# sourceMappingURL=site-gxw4mawm.js.map
