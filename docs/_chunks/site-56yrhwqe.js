...
#endif
`;if(!e.IncludesShadersStore[r])e.IncludesShadersStore[r]=A;var b={name:r,shader:A};var t="openpbrNormalMapFragmentMainFunctions",N=`#if defined(GEOMETRY_NORMAL) || defined(GEOMETRY_COAT_NORMAL) || defined(ANISOTROPIC) || defined(FUZZ) || defined(DETAIL)
...
#endif
`;if(!e.IncludesShadersStore[t])e.IncludesShadersStore[t]=N;var F={name:t,shader:N};var n="openpbrNormalMapFragmentFunctions",L=`#if defined(GEOMETRY_NORMAL)
...
#endif
`;if(!e.IncludesShadersStore[n])e.IncludesShadersStore[n]=L;var D={name:n,shader:L};var a="openpbrConductorReflectance",O=`#define pbr_inline
...
#endif
outParams.F0=1.0;outParams.F90=1.0;return outParams;}`;if(!e.IncludesShadersStore[a])e.IncludesShadersStore[a]=O;var C={name:a,shader:O};var s="openpbrAmbientOcclusionFunctions",P=`float compute_specular_occlusion(const float n_dot_v,const float metallic,const float ambient_occlusion,const float roughness)
...
`;if(!e.IncludesShadersStore[s])e.IncludesShadersStore[s]=P;var M={name:s,shader:P};var c="openpbrVolumeFunctions",x=`struct OpenPBRHomogeneousVolume {vec3 extinction_coeff; 
vec3 ss_albedo; 
...
weight_sum.b<1e-5f ? unconvolved_irradiance.b : irradiance_sum.b/weight_sum.b);}`;if(!e.IncludesShadersStore[c])e.IncludesShadersStore[c]=x;var G={name:c,shader:x};var f="openpbrNormalMapFragment",y=`vec2 uvOffset=vec2(0.0,0.0);
...
#endif
`;if(!e.IncludesShadersStore[f])e.IncludesShadersStore[f]=y;var z={name:f,shader:y};var l="openpbrBlockNormalFinal",U=`#if defined(FORCENORMALFORWARD) && defined(NORMAL)
...
#endif
#endif
`;if(!e.IncludesShadersStore[l])e.IncludesShadersStore[l]=U;var X={name:l,shader:U};var _="openpbrBaseLayerData",w=`vec3 base_color=vec3(0.8);float base_metalness=0.0;float base_diffuse_roughness=0.0;float specular_weight=1.0;float specular_roughness=0.3;vec3 specular_color=vec3(1.0);float specular_roughness_anisotropy=0.0;float specular_ior=1.5;float alpha=1.0;vec2 geometry_tangent=vec2(1.0,0.0);float geometry_thickness=0.0;
#ifdef BASE_WEIGHT
...
#endif
`;if(!e.IncludesShadersStore[_])e.IncludesShadersStore[_]=w;var W={name:_,shader:w};var d="openpbrCoatLayerData",H=`float coat_weight=0.0;vec3 coat_color=vec3(1.0);float coat_roughness=0.0;float coat_roughness_anisotropy=0.0;float coat_ior=1.6;float coat_darkening=1.0;vec2 geometry_coat_tangent=vec2(1.0,0.0);
#ifdef COAT_WEIGHT
...
#endif
`;if(!e.IncludesShadersStore[d])e.IncludesShadersStore[d]=H;var B={name:d,shader:H};var m="openpbrThinFilmLayerData",V=`#ifdef THIN_FILM
...
#endif
`;if(!e.IncludesShadersStore[m])e.IncludesShadersStore[m]=V;var k={name:m,shader:V};var u="openpbrFuzzLayerData",Y=`float fuzz_weight=0.0;vec3 fuzz_color=vec3(1.0);float fuzz_roughness=0.0;
#ifdef FUZZ
#ifdef FUZZ_WEIGHT
...
#endif
#endif
`;if(!e.IncludesShadersStore[u])e.IncludesShadersStore[u]=Y;var Z={name:u,shader:Y};var g="openpbrAmbientOcclusionData",j=`vec3 ambient_occlusion=vec3(1.0);float specular_ambient_occlusion=1.0;float coat_specular_ambient_occlusion=1.0;
#ifdef AMBIENT_OCCLUSION
...
#endif
`;if(!e.IncludesShadersStore[g])e.IncludesShadersStore[g]=j;var q={name:g,shader:j};var p="openpbrBackgroundTransmission",K=`vec4 slab_translucent_background=vec4(0.,0.,0.,1.);
#ifdef REFRACTED_BACKGROUND
...
#endif
`;if(!e.IncludesShadersStore[p])e.IncludesShadersStore[p]=K;var Q={name:p,shader:K};var E="openpbrEnvironmentLighting",J=`#if defined(REFLECTION) || defined(REFRACTED_BACKGROUND)
...
#endif
`;if(!e.IncludesShadersStore[E])e.IncludesShadersStore[E]=J;var ee={name:E,shader:J};var v="openpbrDirectLightingInit",oe=`#ifdef LIGHT{X}
...
#endif
#endif
`;if(!e.IncludesShadersStore[v])e.IncludesShadersStore[v]=oe;var ie={name:v,shader:oe};var h="openpbrDirectLighting",re=`#ifdef LIGHT{X}
...
#endif
`;if(!e.IncludesShadersStore[h])e.IncludesShadersStore[h]=re;var te={name:h,shader:re};var R="openpbrBlockPrePass",ne=`#if SCENE_MRT_COUNT>0
...
#endif
#endif
`;if(!e.IncludesShadersStore[R])e.IncludesShadersStore[R]=ne;var ae={name:R,shader:ne};var I="openpbrPixelShader",$e=`#define OPENPBR_FRAGMENT_SHADER
...
`;if(!e.ShadersStore[I])e.ShadersStore[I]=$e;var eo=[Ee,ve,Pe,pe,T,_e,de,Je,ue,xe,he,Re,Te,Ge,b,be,se,ge,ce,Fe,me,ye,ze,Ue,Ne,Se,Me,Ie,Xe,we,Ce,We,He,Be,F,D,Ae,Ze,C,M,je,qe,G,fe,Ve,z,X,W,Qe,Ke,B,k,Z,q,Le,Q,ee,ie,te,De,le,ke,ae,Oe,Ye];for(let o of eo)if(!e.IncludesShadersStore[o.name])e.IncludesShadersStore[o.name]=o.shader;var rr={name:I,shader:$e};
export{rr as Gi};

//# debugId=71A5EE685781006564756E2164756E21
//# sourceMappingURL=site-56yrhwqe.js.map
