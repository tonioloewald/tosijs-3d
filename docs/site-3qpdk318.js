import{_B as b}from"./site-7jxv124x.js";var f="importanceSampling",q=`vec3 hemisphereCosSample(vec2 u) {float phi=2.*PI*u.x;float cosTheta2=1.-u.y;float cosTheta=sqrt(cosTheta2);float sinTheta=sqrt(1.-cosTheta2);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}
...
float phi=2.*PI*u.x;float sinTheta=pow(u.y,a/(2.*a+1.));float cosTheta=sqrt(1.-sinTheta*sinTheta);return vec3(sinTheta*cos(phi),sinTheta*sin(phi),cosTheta);}`;if(!b.IncludesShadersStore[f])b.IncludesShadersStore[f]=q;var x={name:f,shader:q};var k="hdrFilteringFunctions",v=`#if NUM_SAMPLES
#if NUM_SAMPLES>0
...
#endif
#endif
`;if(!b.IncludesShadersStore[k])b.IncludesShadersStore[k]=v;var A={name:k,shader:v};
export{x as Jj,A as Kj};

//# debugId=37498A620A74C22264756E2164756E21
//# sourceMappingURL=site-3qpdk318.js.map
