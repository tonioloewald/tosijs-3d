...
[ ] b3d demo: the time-of-day scenePanel slider doesn't drive the sky on the FIRST XR entry, but works after you exit and re-enter (works flat either way). Likely a first-session timing issue: the reactive binding demo.time→skybox.timeOfDay is window-rAF-batched and the rAF pump (_installXrRafPump) may miss the first projection (a callback scheduled with the real rAF before the shim, or the panel/skybox binding not warm until the 2nd session). Try: flush/rebuild bound projections right after the pump installs on IN_XR, or ensure the skybox binding re-subscribes on entry. (XR-only; needs a headset to verify.)
