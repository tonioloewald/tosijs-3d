...
[ ] `toolbar3d` widget: a widgets3d row that lays out a series of buttons in a SINGLE row, equally sized. (Emoji work as button glyphs in SVG text — rasterize in modern browsers — but can't be recolored/themed; use SVG paths for themeable icons.)
...
[~] b3d demo: time-of-day slider dead on FIRST XR entry (fix applied, NEEDS HEADSET VERIFY). ROOT CAUSE: tosijs component render is `if(!this._renderQueued){this._renderQueued=true; requestAnimationFrame(...)}` — a per-element flag. If a render is already queued when the immersive session suspends window.rAF, that callback is STRANDED, `_renderQueued` stays true forever, and the component never schedules another render → the pump never sees it → its reactive bindings freeze. The skybox's `realtimeScale` setInterval constantly queues a render, so at entry there's almost always one pending → "never works first time." FIX: `await updates()` in the Enter-VR click handler before `enterXRAsync`, flushing pending renders WHILE flat rAF still works so nothing is stranded. RESIDUAL RACE: a render queued in the last flat frame before IN_XR could still strand → if still flaky, also pause the skybox setInterval during entry or add a pump-level force-flush.
