...
[ ] Galaxy demo: star/nebula sprites don't orient to the XR camera (fine in flat). Sprites are a `SolidParticleSystem` with `billboard = true` (`setParticles()` each frame) + a passthrough vertex shader. SPS billboard faces `scene.activeCamera`; in an XR session that's the `WebXRCamera` (a rig camera) and Babylon's SPS billboard reads `camera.position` (rig-local) rather than `.globalPosition`, so once the rig is offset the facing is wrong. Likely fix: custom `updateParticle` that faces `scene.activeCamera.globalPosition` (XR-safe) instead of the built-in `billboard=true`. NEEDS HEADSET to verify (don't want to regress the flat view).
...
[ ] Library demo (VR): the scrollable panel pushes up and clips the top of the Exit-VR button. Panel/toolbar layout overlap in XR — the scene panel's top edge overruns the enter/exit-VR toolbar. NEEDS HEADSET to tune placement.
