...
- [ ] **Clouds cast shadows** (Tonio) — `castShadows` exists (registers the blobs as casters) but is off by default and untested. Real work to make it good: (1) the sun's shadow range must reach cloud ALTITUDE (activeDistance/shadowMaxZ), which means a big frustum → low-res shadow map; (2) the blobs are OPAQUE, so they'd cast hard-edged ellipse shadows — soft cloud shadows want the shadow generator's transparency/blur. Enable in a demo and tune, or accept coarse blob shadows.
