...
import { table, type Table, type TableRow } from './table.js'
...
const { g, rect, text } = svgElements as any
...
const normalise = (o: PickerOption | string): PickerOption =>
...
export function groupsOf(options: PickerOption[]): string[] {
  const seen: string[] = []
  for (const o of options) {
    const gname = o.group ?? ''
    if (gname !== '' && !seen.includes(gname)) seen.push(gname)
  }
  return seen
}

export function picker3d(config: Picker3dOptions): Picker {
  ...
  const change = handlerOf<(v: string) => void>(
  ...
  const paint = (): void => {
    ...
    while (chevron.firstChild) chevron.removeChild(chevron.firstChild)
    chevron.appendChild(
  ...
  const rowsFor = (group: string): TableRow[] =>
    options
