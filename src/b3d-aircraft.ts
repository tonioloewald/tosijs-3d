...
export class B3dAircraft extends B3dControllable {
  ...
  */
  ...
  getChaseAnchor(): BABYLON.TransformNode | null {
    if (this.owner == null || this.meshNode == null) return null
    if (this._chaseAnchor == null) {
      const anchor = new BABYLON.TransformNode(
        `aircraft-chase-anchor-${this.instanceId}`,
        this.owner.scene
      )
  ...
  private loadFromLibrary(libraryType: string, meshName: string, owner: B3d) {
    ...
    if (!tryLoad()) {
      ...
      */
      const handler = () => {
        if (tryLoad()) this._stopLibraryWait?.()
      }
   ...
   */
  private fitCameras(root: BABYLON.TransformNode): void {
    ...
    const untouched = (v: number, was: number) => Math.abs(v - was) < 1e-6
  ...
  sceneDispose() {
    ...
    if (this._chaseAnchor) {
      ...
      for (const child of this._chaseAnchor.getChildren()) child.parent = null
