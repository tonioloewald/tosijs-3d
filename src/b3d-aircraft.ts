...
export class B3dAircraft extends B3dControllable {
  ...
  applyInput(input: ControlInput, dt: number) {
    ...
    if (!this.grounded) {
      node.rotate(
        BABYLON.Axis.X,
        input.forward * attrs.pitchRate * DEG2RAD * dt,
        BABYLON.Space.LOCAL
      )
  ...
  private updateFlatChase(node: BABYLON.TransformNode) {
    const cam = this.chaseCamera
    if (cam == null || this.cameraView !== 'chase' || this.owner?.xrActive) return
