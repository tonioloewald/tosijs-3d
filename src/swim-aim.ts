 ...
 */
export function aimFromLook(forwardY: number, maxDeg = 70): number {
...
export function integrateAim(
  ...
  if (dt <= 0) return clampAim(currentDeg, maxDeg)
 ...
 */
export function easeAim(
  currentDeg: number,
  targetDeg: number,
  dt: number,
  response = 8
): number {
  if (dt <= 0) return currentDeg
