...
type Preset = {
  ...
  life: [number, number]
  rate: number
  /** Capacity that sustains the look — essentially `rate × maxLife`. What it ASKS for. */
  desired: number
  /** Below this it stops being the thing it claims to be and switches off. 40 raindrops is
   * not light rain, it's a rendering bug. Rain and snow need density to read at all; a few
   * motes still read perfectly well as motes, so their floor is low. */
  min: number
  ...
  dead: BABYLON.Color4
  additive: boolean
  /** Per-axis WANDER. Straight-line particles read as sprites; wandering ones read as matter.
 ...
 */
function dotTexture(scene: BABYLON.Scene): BABYLON.DynamicTexture {
...
export class B3dAmbient extends B3dChild implements AmbientEffect {
  ...
  declare where: 'always' | 'underwater' | 'above'
  declare count: number
  declare minCount: number
  declare minTier: PerfTier
  ...
  get active(): number {
    if (this._leaves != null) return this._leaves.liveCount
  ...
  private _baseRate = 0
  private _granted = 0
  private _id = `ambient-${nextAmbientId++}`
  private _offBudget: (() => void) | null = null
  ...
  private _build(scene: BABYLON.Scene, capacity: number) {
    const p = this._p
    ...
    if (p.wander) {
      ...
      const noise = new BABYLON.NoiseProceduralTexture(
        `ambient-noise-${this.preset}`,
        128,
        scene
      )
  ...
  private _whereWeight(eyeY: number): number {
    ...
    const waterY = this._waterY()
    if (waterY == null) return this.where === 'above' ? 1 : 0
  ...
  private _waterY(): number | null {
    const water = this.owner?.querySelector('tosi-b3d-water') as unknown as {
