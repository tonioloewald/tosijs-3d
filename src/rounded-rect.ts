...
export function roundedRectGeometry(
  ...
  const quad = (x0: number, y0: number, x1: number, y1: number): void => {
    if (x1 - x0 <= 1e-9 || y1 - y0 <= 1e-9) return // degenerate at radius 0
    const a = vert(x0, y0)
    const b = vert(x1, y0)
    const c = vert(x1, y1)
    const d = vert(x0, y1)
  ...
  const cornerFan = (cx: number, cy: number, from: number): void => {
    if (r <= 1e-9) return
 ...
 */
export function signedArea(positions: number[], indices: number[]): number {
  let total = 0
