...
export function lockFillOpacity(
  ...
  if (locked === true) return 0.75 // bold faction fill, inside the white outline
...
export function arcDashArray(
  ...
  for (const [a, b] of clean) {
    ...
    if (last != null && a <= last[1]) last[1] = Math.max(last[1], b)
 ...
 */
export function hudSizePx(width: number, height: number, pct: number): number {
