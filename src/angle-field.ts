...
const { g, circle, path, text, line } = svgElements as any
...
const angleOf = (cx: number, cy: number, x: number, y: number): number =>
  normaliseDegrees((Math.atan2(y - cy, x - cx) / DEG + 90 + 360) % 360)
...
function dialBase(label?: string) {
  ...
  if (label) el.appendChild(caption)
...
const gripDot = (x: number, y: number, r: number, fillColour: string) =>
  circle({
    cx: x,
    cy: y,
    r,
...
/** Dial one direction. */
export function angle3d(config: Angle3dOptions = {}): AngleField {
  ...
  const change = handlerOf<(v: number) => void>(
  ...
  const draw = (): void => {
    ...
    drawTicks(base.ticks as SVGGElement, cx, cy, radius)
    // The permitted sector, when there is a restriction to show.
    const blockedArc = arcComplement(limits)
    base.blocked.setAttribute(
      'd',
      blockedArc ? sectorPath(cx, cy, radius, blockedArc) : ''
    )
    base.fill.setAttribute('d', '')
    while (base.grips.firstChild) base.grips.removeChild(base.grips.firstChild)
    const [hx, hy] = pointAt(cx, cy, radius, value)
    base.grips.appendChild(
      line({
        x1: cx,
        y1: cy,
        x2: hx,
  ...
  const set = (next: number, live: boolean): void => {
...
export function arc3d(config: Arc3dOptions = {}): ArcField {
  const limits = () => ({
  ...
  const change = handlerOf<(v: Arc) => void>(
    config as unknown as Record<string, unknown>,
    'handleChange',
    'onChange'
  )
  ...
  const set = (next: Arc, live: boolean): void => {
    const settled = clampArc(next, limits())
    if (settled.centre === value.centre && settled.width === value.width) return
    value = settled
    draw()
    if (live) change?.(value)
  }
