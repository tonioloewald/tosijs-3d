 ...
 */
const EXTENT_SCALE: Record<Shape['extent'], number> = {
  intimate: 0.5,
  small: 1,
  medium: 3,
  large: 10,
 ...
 */
export function proximityRung(
  distance: number,
  extent: Shape['extent'] = 'small'
): Exclude<Proximity, 'elsewhere'> {
  const s = EXTENT_SCALE[extent]
  const d = Math.max(0, distance)
  if (d <= 0.3 * s) return 'same-spot'
  if (d <= 1.0 * s) return 'contact'
  if (d <= 2.5 * s) return 'reach'
  if (d <= 8 * s) return 'obvious'
  if (d <= 25 * s) return 'noticeable'
  return 'present'
}
 ...
 */
export function routePortals(
  portals: Portal[],
  from: PlaceId,
  to: PlaceId
): { portals: PortalId[]; cost: number } | null {
  if (from === to) return { portals: [], cost: 0 }

  // Bidirectional adjacency over UNLOCKED portals only.
  const adj = new Map<PlaceId, { via: PortalId; to: PlaceId; cost: number }[]>()
  const link = (a: PlaceId, b: PlaceId, via: PortalId, cost: number) => {
  ...
  for (;;) {
    ...
    if (cur == null) return null // nothing left reachable → unreachable
    if (cur === to) break
    done.add(cur)
 ...
 */
export function containmentPath(
  places: Map<PlaceId, Place>,
  placeId: PlaceId
): { id: PlaceId; label: string }[] {
  const chain: { id: PlaceId; label: string }[] = []
  const seen = new Set<PlaceId>()
  let cur: PlaceId | undefined = placeId
