...
export function arcOf(centre: number, width: number): Arc {
  return {
...
export const FULL_CIRCLE: Arc = { centre: 0, width: 360 }

/** Where the arc begins, going anticlockwise-to-clockwise through its centre. */
export const arcStart = (a: Arc): number =>
 ...
 */
export function arcContains(a: Arc, angle: number): boolean {
  if (a.width >= 360) return true
 ...
 */
export function arcWithinArc(inner: Arc, outer: Arc): boolean {
  if (outer.width >= 360) return true
  if (inner.width > outer.width + 1e-9) return false
 ...
 */
export function clampAngleToArc(angle: number, limits: Arc): number {
  if (arcContains(limits, angle)) return normaliseDegrees(angle)
 ...
 */
export function clampArc(value: Arc, limits: ArcLimits = {}): Arc {
  ...
  if (envelope.width >= 360) return arcOf(value.centre, width)
 ...
 */
export function nearestArcGrip(
  a: Arc,
  angle: number,
  reach = 12
): ArcGrip | null {
  const d = (target: number) => Math.abs(wrapDegrees(angle - target))
  const candidates: Array<[ArcGrip, number]> = [
    ['start', d(arcStart(a))],
    ['end', d(arcEnd(a))],
    ['centre', d(a.centre)],
  ]
  let best: ArcGrip | null = null
  let bestD = reach
 ...
 */
export function arcComplement(envelope: Arc): Arc | null {
  if (envelope.width >= 360) return null
