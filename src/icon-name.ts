 ...
 */
export function parseStyleSuffixes(name: string): ParsedIconName | null {
 ...
 */
export function mergeIconStyle(into: IconStyle, add: IconStyle): void {
  if (add.transform) {
  ...
  for (const k of ['opacity', 'fill', 'stroke', 'strokeWidth'] as const) {
    if (add[k] !== undefined && into[k] === undefined) into[k] = add[k]
  }
}
