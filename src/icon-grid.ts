 ...
 */
function defaultCellSize(): number {
  const mm = (
    globalThis as { matchMedia?: (q: string) => { matches: boolean } }
  ).matchMedia
  if (typeof mm !== 'function') return 24
  try {
 ...
 * ```
 */
export function iconGrid3d(config: IconGrid3dOptions): IconGrid {
  ...
  return {
    ...
    handle(kind: PointerKind, x: number, y: number) {
      ...
      if (kind === 'up') {
        ...
        const fired = pressed
        pressed = -1
        if (fired >= 0 && fired === i) activate(fired)
        else draw()
      }
    },
    ...
    */
    focusMove(dx: number, dy: number) {
      ...
      if (dy !== 0) {
        ...
        if (nextRow < 0 || idx >= items.length) return false
        hovered = idx
        draw()
        return true
      }
