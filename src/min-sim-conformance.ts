 ...
 */
function stage(api: MinSimApi): void {
  api.definePlace({
    id: 'world',
    kind: 'world',
    label: 'The World',
  ...
  api.placeEntity(
    { id: 'butler', kind: 'npc', label: 'The Butler' },
    { place: 'study', near: 'player', at: 'reach' }
  )
  api.placeEntity(
    { id: 'wolf', kind: 'npc', label: 'A Wolf' },
    { place: 'garden' }
  )
}
 ...
 */
export function runMinSimConformance(
  makeApi: () => TickableMinSim,
  { describe, test, expect }: ConformanceHarness
): void {
  describe('MinSimApi conformance', () => {
    ...
    test('traverse: changes place + emits placeEntered', () => {
      ...
      expect((entered as { placeId: string }).placeId).toBe('hall')
      // a LOCKED portal is a no-op
      api.placeEntity(
        { id: 'ghost', kind: 'npc', label: 'Ghost' },
        { place: 'study' }
      )
    ...
    test('steer toward an entity closes the distance over ticks', () => {
      ...
      api.placeEntity(
        { id: 'stranger', kind: 'npc', label: 'Stranger' },
        { place: 'garden', near: 'player', at: 'noticeable' }
      )
      const before = api.proximity('player', 'stranger')
      api.steer('stranger', { toEntity: 'player' })
      for (let i = 0; i < 200; i++) api.tick(0.1) // ~20s of approach
