 ...
 */
export function compositeFog(base: FogState, layers: FogLayer[]): FogState {
  const out: FogState = {
    color: { ...base.color },
 ...
 * into it.
 */
export function band(value: number, startAt: number, full: number): number {
  if (startAt === full) return value >= full ? 1 : 0
