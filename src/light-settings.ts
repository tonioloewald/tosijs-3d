 ...
 */
export function lightSettingsSchema(
 ...
 */
export function canonicalLight(s: LightSettings): LightSettings {
 ...
 */
export function validateLight(value: unknown): CurveIssue[] {
  ...
  if (v.kind != null && !['point', 'spot', 'area'].includes(v.kind as string)) {
    issues.push({
      severity: 'error',
  ...
  */
  ...
  for (const key of ['intensity', 'range'] as const) {
    const n = v[key]
