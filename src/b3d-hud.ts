...
export class B3dHud extends B3dChild {
  ...
  projectWorldToHud(
  ...
  ): { x: number; y: number; tracked: boolean } | null {
    ...
    const tgt = BABYLON.Vector3.TransformCoordinates(
      new BABYLON.Vector3(world.x, world.y, world.z),
      inv
    )
    const dz = tgt.z - eye.z
    if (Math.abs(dz) < 1e-6) return null // ray parallel to the glass
    const t = -eye.z / dz // where the eye→target ray crosses the plane (z = 0)
    if (t <= 0) return null // the glass is behind us relative to the target
  ...
  private _projectViaScreen(
    ...
    const scene = this.owner?.scene
    const svg = this.controller?.el as SVGSVGElement | undefined
    ...
    if (p.z < 0 || p.z > 1) return null // behind the eye
    const cr = canvas.getBoundingClientRect()
    const sr = svg.getBoundingClientRect()
    if (sr.width < 1 || sr.height < 1) return null // overlay not laid out
  ...
  setTraces(
    ...
    for (const t of traces) {
      ...
      if (p == null) continue // behind the glass — not shown
      points.push({
        x: p.x,
        y: p.y,
        kind: t.kind,
