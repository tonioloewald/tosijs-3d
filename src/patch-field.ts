 ...
 * `composeLandforms`. */
export function composePatches(...patches: PatchField[]): PatchField {
  return (x, y, z, d) => {
    let acc = d
    for (const p of patches) acc = p(x, y, z, acc)
    return acc
  }
}

/** A circular footprint (the common case: a vent, a shaft, a chamber). */
export function circleFootprint(
  cx: number,
  cz: number,
  radius: number
): Footprint {
...
export function marginBlend(
  ...
  return (x, y, z, d) => {
    ...
    if (sd >= 0) return d // outside: the terrain is none of this patch's business
    const w = smoothstep(-sd / m)
    if (w <= 0) return d + tuck
