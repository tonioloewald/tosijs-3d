...
export const NO_WIND: Wind = { x: 0, z: 0 }
...
export function windToPolar(wind: Wind): {
  speed: number
  bearingDeg: number
} {
...
export const scaleWind = (wind: Wind, k: number): Wind => ({
  x: wind.x * k,
  z: wind.z * k,
})
...
const smoothFalloff = (t: number): number => {
  const u = 1 - Math.min(1, Math.max(0, t))
  return u * u * (3 - 2 * u)
}

/** How strongly a province acts at a point. `0` outside its radius. */
export function provinceInfluence(
  province: ProvinceWind,
  x: number,
  z: number
): number {
  if (!(province.radius > 0)) return 0
...
export function windAt(
  base: Wind,
  provinces: readonly ProvinceWind[],
  x: number,
  z: number
): Wind {
  let out = { ...base }
...
export function gustAt(
  wind: Wind,
  t: number,
  options: { amount?: number; period?: number; seed?: number } = {}
): Wind {
  const amount = options.amount ?? 0
  if (amount <= 0) return { ...wind }
  const period = options.period ?? 6
  const speed = windSpeed(wind)
  if (speed < 1e-9) return { ...wind }
...
const gustFields = new Map<number, PerlinNoise>()
function gustNoise(seed: number): PerlinNoise {
 ...
 */
export function waterWind(wind: Wind): {
  windForce: number
