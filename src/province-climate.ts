...
export const DEFAULT_AMOUNTS: Required<ClimateAmounts> = {
  water: 0.35,
  temperature: 0.3,
  volcanism: 1,
}
...
export const NO_CLIMATE: ClimateSample = {
  water: 0,
  temperature: 0,
  volcanism: 0,
}

const CHANNELS = ['water', 'temperature', 'volcanism'] as const

function sampleCurve(
  curve: ClimateCurve | undefined,
  at: number,
  fallback: number
): number {
...
export function sampleClimate(
  climate: ProvinceClimate | null | undefined,
  t: number
): ClimateSample {
  if (climate == null) return NO_CLIMATE
  const amounts = { ...DEFAULT_AMOUNTS, ...climate.amounts }
  const out: ClimateSample = { water: 0, temperature: 0, volcanism: 0 }
 ...
 */
export function composeClimate(samples: ClimateSample[]): ClimateSample {
  const out: ClimateSample = { water: 0, temperature: 0, volcanism: 0 }
...
export function applyClimate(
  axes: { temperature: number; moisture: number },
...
export function provinceClimateSchema(
  extra: Record<string, unknown> = {}
): Record<string, unknown> {
  const channel = (title: string) => ({
    ...curveSchema('falloff'),
    title,
...
export function canonicalClimate(c: ProvinceClimate): ProvinceClimate {
  const r = (v: number) => Math.round(v * 1e4) / 1e4 + 0
  const out: ProvinceClimate = {}
  for (const key of CHANNELS) {
    const curve = c[key]
    if (curve == null) continue
  ...
  if (c.amounts != null) {
    const a: ClimateAmounts = {}
    for (const key of CHANNELS) {
      const v = c.amounts[key]
      if (v != null) a[key] = r(v)
    }
    if (Object.keys(a).length > 0) out.amounts = a
  }
  if (c.hasWater != null) out.hasWater = c.hasWater
  if (c.waterLevel != null) out.waterLevel = r(c.waterLevel)
  return out
}

/** Report what is wrong without throwing or fixing it. Paths are RELATIVE. */
export function validateClimate(value: unknown): CurveIssue[] {
  ...
  for (const key of CHANNELS) {
    const curve = c[key]
    if (curve == null || typeof curve === 'number') continue
