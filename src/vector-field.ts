...
function show(n: number, precision: number): string {
...
function coordinateRow(
  config: Vector3dOptions,
  settle: (n: number) => number
): VectorField {
  ...
  const current: Vec3 = { ...(config.value ?? { x: 0, y: 0, z: 0 }) }
  ...
  */
  ...
  const fields = keys.map((key, i) =>
    inputField({
      ...
      handleChange: (raw: string) => {
        ...
        if (v !== n) fields[i].setValue(show(v, precision))
        handlerOf<(v: any) => void>(
  ...
  let cols: Array<{ x: number; width: number }> = []
  let rowHeight = 0
  ...
  */
  ...
  const at = (x: number, y: number) => {
    for (let i = 0; i < cols.length; i++) {
      const c = cols[i]
      if (x >= c.x && x <= c.x + c.width) return { i, lx: x - c.x, ly: y }
    }
    return null
  }
 ...
 * ```
 */
export function vector3d(config: Vector3dOptions = {}): VectorField {
 ...
 * ```
 */
export function euler3d(config: Vector3dOptions = {}): VectorField {
