...
/** The element shape we write a transform to, when the target is one. */
...
export class B3dManipulator extends B3dChild {
  ...
  private _view: HandlesView | null = null
  private _drag: Drag | null = null
  private _pointer: BABYLON.Nullable<BABYLON.Observer<BABYLON.PointerInfo>> =
    null
  private _frame: BABYLON.Nullable<BABYLON.Observer<BABYLON.Scene>> = null
  ...
  */
  private _transformSet(): TransformSet {
  ...
  private _targetElement(): TransformableElement | null {
    const selector = String(this.target ?? '')
    if (selector === '' || this.owner == null) return null
    const found =
      this.owner.querySelector(selector) ??
      (this.getRootNode() as ParentNode).querySelector?.(selector) ??
      null
    return (found as TransformableElement) ?? null
  }

  /** The node we move, from whichever route was used to name it. */
  private _targetNode(): BABYLON.TransformNode | null {
    if (this.node != null) return this.node
    const el = this._targetElement()
    if (el?.mesh != null) return el.mesh
    const name = String(this.meshName ?? '')
    if (name !== '' && this.owner != null) {
      return this.owner.scene.getNodeByName(
        name
      ) as BABYLON.TransformNode | null
    }
    return null
  }
  ...
  private _track(): void {
    ...
    const scene = this.owner?.scene
    if (view == null || scene == null) return
  ...
  private _rotationOf(node: BABYLON.TransformNode): {
    rx: number
    ry: number
    rz: number
  } {
  ...
  private _toWorldSpace(
    ...
    const parent = node.parent as BABYLON.TransformNode | null
  ...
  private _currentTransform(node: BABYLON.TransformNode): ManipulatorTransform {
    /*
  ...
  grab(ray: ManipulatorRay, options: { secondary?: boolean } = {}): boolean {
    const view = this._view
    const node = this._targetNode()
    if (view == null || node == null || this.disabled === true) return false
  ...
  private _begin(
    ...
    const drag = beginDrag(
      ...
      transform,
      ray,
      axisFrameOf(transform.rotation),
  ...
  private _write(t: ManipulatorTransform): void {
    const node = this._targetNode()
    if (node == null) return
    const el = this._targetElement()
    ...
    if (el != null && typeof el.x === 'number') {
      ...
      */
      ...
      node.rotationQuaternion = BABYLON.Quaternion.RotationYawPitchRoll(
        t.rotation.ry * DEG_TO_RAD,
        t.rotation.rx * DEG_TO_RAD,
        t.rotation.rz * DEG_TO_RAD
      )
    }
  ...
  private _onPointer(info: BABYLON.PointerInfo, scene: BABYLON.Scene): void {
    if (this.disabled === true) return
    const camera = scene.activeCamera
    if (camera == null) return
    const e = info.event as PointerEvent
    const pick = scene.createPickingRay(
      scene.pointerX,
      scene.pointerY,
      BABYLON.Matrix.Identity(),
      camera
    )
    const ray: ManipulatorRay = {
    ...
    if (
      info.type === BABYLON.PointerEventTypes.POINTERUP ||
