...
export class B3dLibrary extends B3dChild {
  ...
  private _allNodes(): BABYLON.Node[] {
    ...
    return [...this.container.meshes, ...this.container.transformNodes].filter(
      (n) => n.name !== '__root__' && !isIgnored(n.name)
    )
  }
  ...
  getHierarchy(): {
    ...
    const buildTree = (
      ...
      return allNodes
        .filter(
          (n) =>
            n.parent === parent && n.name !== '__root__' && !isIgnored(n.name)
        )
        .map((n) => {
  ...
  instantiate(
    ...
    const resolved = resolveModelName(
      [...this.container.meshes, ...this.container.transformNodes].map(
        (n) => n.name
      ),
      name
    )
    const source: BABYLON.Node | undefined =
      this.container.meshes.find((m) => m.name === resolved) ??
    ...
    if (result instanceof BABYLON.TransformNode) {
      ...
      */
      if (
        options.rx !== undefined ||
        options.ry !== undefined ||
        options.rz !== undefined
      ) {
        const DEG = Math.PI / 180
        result.rotationQuaternion = BABYLON.Quaternion.RotationYawPitchRoll(
          (options.ry ?? 0) * DEG,
          (options.rx ?? 0) * DEG,
          (options.rz ?? 0) * DEG
        )
