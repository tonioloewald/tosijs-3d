...
export function terrainEditor3d(
  ...
  const current = (key: string): any => s[key] ?? props[key]?.default

  /** One control, its shape decided entirely by the schema entry. */
  const control = (key: string): Widget3d | null => {
    const p = props[key]
    ...
    */
    ...
    return slider3d({
      ...
      max: p.maximum ?? 100,
      scale: usable ? (p['x-scale'] as 'log' | 'log2') : 'linear',
      zeroStop: usable && zeroStop,
      snap: p['x-snap'],
      showValue: 'always',
      // The unit is the schema's, so a frequency reads as one.
      format: (v: number) =>
        p['x-unit'] != null ? `${trim(v)} ${p['x-unit']}` : trim(v),
      handleChange: (v) => {
        s[key] = v
        emit(label, true)
      },
  ...
  const group = (title: string, keys: string[]): Widget3d[] => {
    const made = keys.map(control).filter((w): w is Widget3d => w != null)
  ...
  const api: TerrainEditorField = {
    ...
    setHost(host: WidgetHost) {
      ...
      for (const { w, top } of tops) {
        w.setHost?.({
          ...host,
          showPopup: (cfg: any, ...items: any[]) =>
            host.showPopup(
              { ...cfg, anchor: { ...cfg.anchor, y: cfg.anchor.y + top } },
              ...items
            ),
        } as WidgetHost)
      }
  ...
  function rowAt(y: number): { w: Widget3d; top: number } | undefined {
