 ...
 */
export function parseGlbJson(bytes: ArrayBuffer | Uint8Array): any | null {
  const u8 = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes)
  if (u8.byteLength < 20) return null
  const dv = new DataView(u8.buffer, u8.byteOffset, u8.byteLength)
  if (dv.getUint32(0, true) !== MAGIC) return null
  const jsonLength = dv.getUint32(12, true)
  if (dv.getUint32(16, true) !== JSON_CHUNK) return null
  ...
  if (20 + jsonLength > u8.byteLength) return null
  try {
    return JSON.parse(
      new TextDecoder().decode(u8.subarray(20, 20 + jsonLength))
    )
  } catch {
    return null
  }
 ...
 */
export function glbJsonByteLength(
 ...
 */
export function libraryManifest(json: any): LibraryManifest | null {
  const lib = json?.scenes?.[0]?.extras?.library
  if (lib == null || !Array.isArray(lib.items)) return null
  const items: LibraryItem[] = lib.items.filter(
    (i: any) => i != null && typeof i.name === 'string' && i.name !== ''
  )
 ...
 */
export function manifestFromNodes(
  nodes: Array<{ name: string; metadata?: any }>
): LibraryManifest | null {
  const items: LibraryItem[] = []
  const categories: Record<string, number> = {}
  for (const n of nodes) {
    const extras = n?.metadata?.gltf?.extras
    if (extras == null || typeof extras !== 'object') continue
