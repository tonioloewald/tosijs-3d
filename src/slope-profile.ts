...
export function cliffProfile(shelf = 0.35, rise = 0.12): PiecewiseLinearFilter {
...
export function beachProfile(shelf = 0.55): PiecewiseLinearFilter {
  return new PiecewiseLinearFilter([
    { x: 0, y: 0 },
    { x: shelf, y: 0.16 },
    { x: 0.85, y: 0.62 },
...
export function rollingProfile(softness = 0.5): PiecewiseLinearFilter {
...
export function mesaProfile(steps = 4): PiecewiseLinearFilter {
...
export function terraceProfile(steps = 4, tread = 0.6): PiecewiseLinearFilter {
...
export function blendProfiles(
  ...
  const evalA = (t: number, x: number, z: number) =>
    (a as LocalizedFilter).evaluateAt?.(t, x, z) ?? a.evaluate(t)
  const evalB = (t: number, x: number, z: number) =>
    (b as LocalizedFilter).evaluateAt?.(t, x, z) ?? b.evaluate(t)
  return {
    ...
    evaluateAt(t: number, x: number, z: number): number {
      ...
      if (cw <= 0) return evalA(t, x, z)
      if (cw >= 1) return evalB(t, x, z)
