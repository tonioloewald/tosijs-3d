...
| `GALAXY-DESIGN.md`         | **The voxel galaxy** — every voxel has two derived seeds (bright / dim); density sampled from the current generator, propagated so no voxel is empty; bright stars global, dim stars and small nebulae generated locally; stars addressed by `(voxel, population, n)`. Design only, nothing built yet                                                                                                                                                                        |
