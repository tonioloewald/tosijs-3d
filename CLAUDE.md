...
| Doc                        | What it holds                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **the virta board**       | **Where the work lives** (since 2026-09-26): open bugs, "needs a headset" items, in-flight designs. A session opens with `virta brief` (hook); `virta ls "project:tosijs-3d status:ready"` is what is cleared to work on. Agents file into backlog; `ready` is Tonio's go-ahead. `TODO.md` is only a pointer — keep it prose, a list item there gets imported as a task |
| `DECISIONS.md`             | The design journal that used to be `TODO.md`, verbatim — measurements, decisions, what was ruled out and why. Read it for the reasoning; its checkboxes are frozen. Every item in it is also a board card linking back to its line |
...
| `TUNNEL-DESIGN.md`         | Caves/tunnels — **topology → voxel map → density → geometry**; the voxel map is the geometry source of truth, topology compiles into it, and connectivity is VERIFIED against the declared graph                                                                                                                                                                                                                                                                             |
| `MINIATURES-DESIGN.md`     | **Recovered NOTES, not a design** — detail from the Amiga original, captured before it evaporated. The game will be its own project and Tonio writes its design. What is actionable HERE is the last section: the framework capabilities those mechanics imply (parameterised projectiles, cell occupancy, role-carrying destroyables), each of which a second game would also want                                                                                          |
| `PROVINCE-DESIGN.md`       | **Provinces** — a place that differs from its planet: one footprint and ONE falloff, with optional shape / material / volume / decoration / climate layers, and the composition rule for each. The unifying architecture, mostly unbuilt                                                                                                                                                                                                                                     |
| `MEDIUM-DESIGN.md`         | **Medium** — one idea for water/air/cloud/weather/vacuum: the shipped primitive, then the staged generalisation (optics → sky-as-medium → shafts → transition whiteouts) and what would falsify it                                                                                                                                                                                                                                                                           |
| `WEATHER-DESIGN.md`        | **Weather** — one question every component asks: `weatherAt(x, z, t)` (wind, coverage, precipitation, temperature, storminess) = base + climate provinces + SYSTEMS (a front is a province whose centre moves). Consumers read it at their position; lightning and light shafts are ONE subsystem (the weather's light). Design, for review |
| `MOBILITY-DESIGN.md`       | **Mobility** — the biped bifurcates into a _platform jumper_ (instant verbs) and an _intent model_ (steer it, it solves the terrain). North star: cover you **discover**, never a mode you are stuck in; derived like `isSwimming`, not entered                                                                                                                                                                                                                              |
| `SPATIAL-DESIGN.md`        | Spatial attachment — attach / place-relative / transition (riding an elevator), the pure transform math                                                                                                                                                                                                                                                                                                                                                                      |
| `COLLISION-DESIGN.md`      | **Probes** — the reusable "is this hitting that" test takes a POSE SOURCE, never a position (so a bone-attached sword tip, a CoG pivot and a floating-origin shift are all one case); why skinned picking is the bind pose; declarative exclusion groups                                                                                                                                                                                                                     |
...
| `GALAXY-DESIGN.md`         | **The voxel galaxy** — every voxel has two derived seeds (bright / dim); density sampled from the current generator, propagated so no voxel is empty; bright stars global, dim stars and small nebulae generated locally; stars addressed by `(voxel, population, n)`. ONE galaxy since 2026-09-25: `generateGalaxy` is a deprecated adapter over it; stars addressed `seed:population:voxel:n`; dim-voxel streaming in the live galaxy not yet                                                                                                                                                                        |
...
| `RELEASING.md`             | Release checklist (run steps 1–7; stop before `npm publish`). **Minors/majors add step 5a: a human drives every demo flat AND in VR** — the combination bugs that reach adopters cannot fail a unit test                                                                                                                                                                                                                                                                     |
| `CHANGELOG.md`             | Per-version Added/Changed/Fixed, with a **⚠️ Breaking** block for any peer-dependency range change                                                                                                                                                                                                                                                                                                                                                                           |
| `UPSTREAM.md`              | Pointer: what we wait on in other repos is on the board as `notes:upstream` cards (`fixed-upstream` = fixed there, workaround not yet walked). File, don't fix. The old table is at the end of `DECISIONS.md` |
...
| `static/attributes.txt`    | **Generated attribute index** — one greppable line per element attribute, rebuilt from source by `site.config.ts`'s prebuild. (Counts are deliberately not recorded here: two gates in a row filed them as stale.) Exists because the recurring adopter failure is a feature that ships, is documented, and cannot be found. `grep water` finds `submersible`; reading 147 pages does not. Also emitted as `attributes.json`. Never hand-edit — see `bin/attribute-index.ts` |
...
| `src/touch-gesture.ts` / `src/touch-orbit.ts` | Two-finger touch on an orbit camera: PAN on a drag, ZOOM on a pinch (tosijs-3d#52). `touch-gesture` is the pure decision model (unit-tested); `touchOrbit(camera)` wires it, and the default camera uses it |
...
| `src/weather.ts` / `src/b3d-weather-cell.ts` | Weather stage 1 (WEATHER-DESIGN.md): the pure `weatherAt(base, cells, x, z)` with per-quantity composition, and `<tosi-b3d-weather-cell>` (a region that differs; `drift="wind"` + `lifetime` make it a system). `b3d.weatherAt`/`weatherHere` is the scene service consumers read AT THEIR POSITION |
| `src/lightning.ts` / `src/b3d-lightning.ts` | Weather stage 3: the pure seeded strike scheduler (`strikesBetween`, aligned slots so intervals split exactly), `boltPath`, `flashAt` (re-strokes), `thunderDelay`; the element draws the flash (deck lit from inside via `deck.flash()`, a ground light), bolts, sprites, procedural thunder, and dispatches `strike` |
...
| `src/light-modulation.ts` | Pure model behind a lamp's flicker/pulse/fade — **ONE `[0,1]→[0,1]` curve per channel spanning the lamp's whole life**, split into attack / sustain / decay by two markers (`attackEnd`, `sustainEnd`). The seams cannot jump because there is only one curve, so the value AT `attackEnd` _is_ the sustain level. Flicker deliberately does NOT compose across regions (draw the tail you want); the two accepted discontinuities are shut-off-mid-loop and shut-off-while-spinning-up. brightness/range/saturation multiply, hue shifts bipolar about 0.5. Unit-tested |
| `src/scene-schemas.ts` | **JSON Schema for the scene primitives** (skybox/sun/water/fog/clouds/ambient/light) — no DOM, no Babylon, so an adopter reads them headlessly. Exists because a hand-copied schema drifts: ensemble's carried 6 of skybox's 16 attributes and disagreed on `applyFog`'s default. Carries NO `x-widget` (these are ordinary controls); carries what `initAttributes` cannot — ranges, units, enums, log hints. `SCENE_OMITTED` makes declining an attribute explicit, and `scene-schemas.test.ts` fails on any drift |
...
| `src/scatter.ts` | Pure placement behind the decorator: world-anchored cells + a bisection threshold that hits the budget exactly, soft climate/slope/altitude bands (a hard floor at sea level), province suppression by kind, `NATURE_KIT_RULES`, and `NearIndex` — the one near-set for everything that exists only near the viewer. Unit-tested |
...
| `src/province-climate.ts` | What a province does to the WEATHER — water / temperature / volcanism as **bipolar** curves over normalised distance (`0.5` leaves the base alone), plus the two water SCALARS, because "there is a lake here at 12 m" is not something a curve can say. Temperature and water SUM across provinces (signed, so a lake beside a lava field cancels); volcanism takes the MAX. Pure, unit-tested |
...
| `src/star-populations.ts` / `population-table.ts` / `spectral-classes.ts` | The "interesting stars" population (HI ≤ 2): pass-rate-biased candidate mixes, verified with the real HI rules (`bestHIOf`). `population-table.ts` is GENERATED by `bin/tune-populations.ts` (rerun the harness when the HI rules change); `spectral-classes.ts` is the leaf holding the spectral tables, split out to break the galaxy-data ↔ voxel-galaxy cycle |
...
| File                                           | Purpose                                                                                                                                                                                                                                                                                                                |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/color.ts`                                 | Pure colour model — parse (tolerant) / format (canonical `#rrggbb[aa]`), RGB↔HSV, `contrastInk`. No DOM, so a consumer validates a document's colours headlessly                                                                                                                                                       |
| `src/color-field.ts`                           | **`color3d`** — the SVG colour picker: S×V square, hue strip, alpha over a chequerboard. The one control the SVG UI lacked, and both previous routes were DOM (so neither existed in a headset)                                                                                                                        |
...
| `src/curve-program.ts`                         | `curveProgram3d` — several curves sharing ONE pair of split markers, as one field. Deliberately not six schema fields: the markers belong to the PROGRAM, so siblings could be edited into disagreement and the document would hold a lamp that cannot run. Controlled; one `handleCommit` per gesture = one undo step |
| `src/light-editor.ts`                          | `lightEditor3d` — the WHOLE lamp: power switch, type, colour (HSV — no value slider, because value IS intensity), range, shadows, plus the program editor. Colour is hue+saturation rather than RGB because "the same colour, a bit warmer" is the edit you actually make on a light                                   |
| `src/light-settings.ts`                        | A lamp as **DATA** — type, defaults, JSON Schema, canonical form, validator. **No DOM, no Babylon**: importing the EDITOR pulls in tosijs and needs `HTMLElement`, which put `validateLight` behind a browser, and adopters validate documents headlessly. Same split as `world-store`/`world-view`                    |
...
| `src/popup-chrome.ts`                          | Pure popup title-bar geometry — where the move/close glyphs go, and where the close TARGET is (`closeHitX`, sized independently of the glyph so shrinking the × does not shrink what you have to hit). Unit-tested                                                                                                     |
...
| `src/patch-field.ts` | `landform`'s VOLUMETRIC sibling (kept: `terrainDensity` is how a density field agrees with the heightfield) — `(x,y,z,d) => d'` patches that carve (bores/caverns), `terrainDensity` from the hooked height sampler, `marginBlend` so a rim converges onto the heightfield (tucked below it) instead of being stitched. Pure, unit-tested |
...
attributes ([tosijs#27], closed — a prototype accessor survives on tosijs 1.9,
verified against 1.9.2: setting `turnRateDeg` runs the setter and `turnRate`
follows, while `observedAttributes` still lists only the stored name).
...
- **before 1.9** — a wrong-typed write was silently DISCARDED, so a leftover `foo: false` compiled, ran, and quietly meant `'on'`. First link in the lost-pointerup saga (see UI-DESIGN-NOTES).
- **1.9 and later** — it WARNS and applies the value as given, naming the prop, both types and the issue. Nothing is coerced.

Better, but not harmless: `foo: false` on an `'on'|'off'` attribute now genuinely sets `false`, so anything comparing against the string breaks instead of silently defaulting. Use `isOff()` from `b3d-utils`, which accepts `'off'`, `false` and `'false'` for exactly this reason.
