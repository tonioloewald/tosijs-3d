...
| `MINIATURES-DESIGN.md`     | **Miniatures battle** — recovered from the Amiga original: grid occupancy with sub-cell pose, projectiles as `(launch, landing, t)`, units that RE-FORM on arrival (so transit is vulnerable), and command as a flag bearer you can kill. Ends with the warning that matters most — it died of featuritis once |
