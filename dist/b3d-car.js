...
export class B3dCar extends B3dControllable {
    ...
    applyInput(input, dt) {
        ...
        // Clamp speed
        this.speed = Math.max(-attrs.maxSpeed * 0.3, Math.min(attrs.maxSpeed, this.speed));
        // Steering: turn rate depends on speed (no turning while stopped)
        const speedFactor = Math.min(1, Math.abs(this.speed) / 3);
        const steer = input.turn * attrs.turnRate * speedFactor * dt * (Math.PI / 180);
        ...
        if (attrs.url !== '') {
            this.loadAssetContainer(scene, attrs.url, (container) => {
                ...
                if (this.entries.rootNodes.length !== 1) {
                    throw new Error('<tosi-b3d-car> expects a container with exactly one root node');
                }
