...
import { Color3, Matrix, Mesh, MeshBuilder, Quaternion, Ray, StandardMaterial, Vector3, } from '@babylonjs/core';
...
function rotated(v, q) {
    const out = Vector3.Zero();
    new Vector3(v.x, v.y, v.z).rotateByQuaternionToRef(q, out);
    return { x: out.x, y: out.y, z: out.z };
}
const quatOf = (rotation) => Quaternion.RotationYawPitchRoll(rotation.ry * DEG, rotation.rx * DEG, rotation.rz * DEG);
/**
 ...
 */
export function composeRotation(start, axis, degrees) {
    const base = quatOf(start);
    const local = Quaternion.RotationAxis(axis === 'x'
        ? Vector3.Right()
        : axis === 'y'
            ? Vector3.Up()
            : Vector3.Forward(), degrees * DEG);
 ...
 */
export function axisFrameOf(rotation) {
    if (rotation == null ||
 ...
 */
export function createHandles(scene, scale = 1) {
    const handles = [];
    const materials = [];
    ...
    const material = (key, colour, alpha = 1) => {
     ...
     */
    const quarterAnnulus = (name, inner, outer, startDegrees) => {
        const arc = (radius) => {
            const points = [];
        ...
        return MeshBuilder.CreateRibbon(name, { pathArray: [arc(inner), arc(outer)], sideOrientation: Mesh.DOUBLESIDE }, scene);
    };
    /**
     ...
     */
    const arcStart = (axis) => axis === 'x' ? 225 : axis === 'z' ? 45 : 135;
    /**
        ...
        for (const axis of ['x', 'y', 'z']) {
            ...
            if (transforms.translate) {
                add(grip('translate'), {
                    ...
                    spin: alongAxis(axis),
                    make: (name, fat) => squareTapered(name, {
                        height: SHAFT_LENGTH,
                        diameterTop: SHAFT_DIAMETER * (fat > 1 ? SHAFT_PICK_FATNESS : 1),
                        diameterBottom: SHAFT_DIAMETER * (fat > 1 ? SHAFT_PICK_FATNESS : 1),
                    }),
                });
                add(grip('translate'), {
                    part: 'head',
                ...
                */
                add(grip('planar'), {
                    ...
                    spin: facing(axis),
                    make: (name, fat) => MeshBuilder.CreatePlane(name, { size: PAD_SIZE * (fat > 1 ? 1.6 : 1) }, scene),
                });
            }
            if (transforms.rotate) {
                add(grip('rotate'), {
                    colour,
                    spin: ringOn(axis),
                    make: (name, fat) => quarterAnnulus(name, fat > 1 ? RING_INNER - RING_PICK_MARGIN : RING_INNER, fat > 1 ? RING_OUTER + RING_PICK_MARGIN : RING_OUTER, arcStart(axis)),
                });
            }
            if (transforms.scale) {
                add(grip('scale'), {
                    ...
                    make: (name, fat) => MeshBuilder.CreateBox(name, { size: CUBE_SIZE * (fat > 1 ? 2.2 : 1) }, scene),
                });
            }
        }
        if (transforms.scale) {
            add({ kind: 'uniform' }, {
                colour: NEUTRAL,
                make: (name, fat) => MeshBuilder.CreateBox(name, { size: CENTRE_SIZE * (fat > 1 ? 2 : 1) }, scene),
            });
        }
        place();
    };
    ...
    */
    const place = () => {
        for (const { grip, mesh, offset, spin } of handles) {
            ...
            */
            const local = orientation != null && (grip.kind === 'scale' || grip.kind === 'rotate');
            ...
            if (local) {
                ...
                mesh.rotationQuaternion = orientation.multiply(Quaternion.RotationYawPitchRoll(spin.y, spin.x, spin.z));
            }
            else {
                mesh.rotationQuaternion = null;
            }
            mesh.position.set(position.x + at.x * scale, position.y + at.y * scale, position.z + at.z * scale);
            /*
    ...
    const isDrawn = (mesh) => mesh?.metadata?.[DRAWN_TAG] === true;
    build();
    return {
        setTransforms(next) {
            if (next.translate === transforms.translate &&
        ...
        setOrientation(rotation) {
            const next = rotation != null &&
                (rotation.rx !== 0 || rotation.ry !== 0 || rotation.rz !== 0)
                ? quatOf(rotation)
                : null;
            const same = (next === null && orientation === null) ||
                (next !== null &&
                    orientation !== null &&
        ...
        nearestGrip(hand) {
            ...
            */
            ...
            for (const { grip, mesh } of handles) {
                const d = Math.hypot(mesh.position.x - hand.x, mesh.position.y - hand.y, mesh.position.z - hand.z);
                if (d <= bestDistance) {
                    bestDistance = d;
                    best = grip;
                }
        ...
        gripAt(ray) {
            if (handles.length === 0)
                return null;
            const r = new Ray(new Vector3(ray.origin.x, ray.origin.y, ray.origin.z), new Vector3(ray.direction.x, ray.direction.y, ray.direction.z));
...
//# sourceMappingURL=manipulator-view.js.map