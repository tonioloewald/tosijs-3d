 ...
 */
function proceduralNebula(scene) {
    const tex = new BABYLON.DynamicTexture('nebula-falloff', { width: 128, height: 128 }, scene, false);
 ...
 */
function starDecodeGlsl(palette) {
    return `
${palette}
...
vec3 b3dUnpackOne(float bits, float slot, float face, vec2 texel, vec3 viewDir) {
  ...
  float c = mod(v, 2.0);
  float brightness = pow((bb + 1.0) / 8.0, DISPLAY_EXP / ${BRIGHT_GAMMA.toFixed(3)});
...
function registerForkedSky() {
    const store = BABYLON.ShaderStore.ShadersStore;
    if (store[`${B3D_SKY}PixelShader`] != null)
        return true;
    const src = store.skyPixelShader;
    const vert = store.skyVertexShader;
    if (src == null || vert == null)
        return false;
    ...
    store[`${B3D_SKY}VertexShader`] = vert
        .replace('varying vec3 vPositionW;', 'varying vec3 vPositionW;\nvarying vec3 vSkyLocal;')
...
*/
const MOONS_GLSL = 'vec3 b3dMoonsAt(vec3 d,out float occ){vec3 c=vec3(0.0);occ=0.0;' +
    ...
    'return c;}';
function makeForkedSkyMaterial(scene) {
    const mat = new BABYLON.ShaderMaterial('b3d-skybox', scene, { vertex: B3D_SKY, fragment: B3D_SKY }, {
        attributes: ['position'],
        uniforms: [
            'world',
            'viewProjection',
...
export class B3dSkybox extends AbstractMesh {
    ...
    _veilColor = new BABYLON.Color3(1, 1, 1);
    ...
    */
     ...
     * than occlude.
     */
    _buildNebulae(scene, prng, radius) {
        const attrs = this;
        const count = Math.floor(attrs.nebulae) || 0;
        if (count <= 0 || this._starfieldMesh == null)
            return;
        /*
        ...
        */
        ...
        const size = radius * 2 * attrs.nebulaSize;
        for (let i = 0; i < count; i++) {
            const z = prng.realRange(-1, 1);
            const t = prng.realRange(0, Math.PI * 2);
            const r = Math.sqrt(Math.max(0, 1 - z * z));
            const quad = BABYLON.MeshBuilder.CreatePlane(`skybox-nebula-${i}_nocast`, { size: size * prng.realRange(0.6, 1.6) }, scene);
            quad.position.set(radius * r * Math.cos(t), radius * z, radius * r * Math.sin(t));
            /*
            ...
            */
            ...
            quad.rotate(BABYLON.Axis.Z, prng.realRange(0, Math.PI * 2), BABYLON.Space.LOCAL);
            /*
            ...
            */
            ...
            const V = 0.12;
            mat.emissiveColor = new BABYLON.Color3(V * (0.35 + 0.65 * warm), V * (0.2 + 0.3 * (1 - Math.abs(warm - 0.5) * 2)), V * (0.4 + 0.6 * (1 - warm)));
    ...
    _excludeFromGlow(scene) {
        const mesh = this._starfieldMesh;
        if (mesh == null || this._glowExcluded)
            return;
    ...
    */
    ...
    _builtStarfieldKey = '';
    _starfieldKey() {
        return B3dSkybox.STARFIELD_KEYS.map((k) => String(this[k])).join('|');
    }
    ...
    */
    _applyStarLook() {
        if (this._starData == null || this._starTexels <= 0)
            return;
        const mat = this.mesh?.material;
        if (mat?.setVector4 == null)
            return;
        const attrs = this;
        const n = this._starTexels;
        mat.setVector4('b3dStarInfo', new BABYLON.Vector4(n, Math.PI / 2 / n, Number(attrs.starfieldSharpness) || 1, Number(attrs.starfieldSizeScale) || 3));
    ...
    _disposeStarfield() {
        ...
        this._starData = null;
        for (const q of this._nebulaMeshes)
            q.dispose();
        this._nebulaMeshes = [];
        for (const nm of this._nebulaMats)
            nm.dispose();
        this._nebulaMats = [];
        this._nebulaBase = [];
    ...
    _buildStarfield(scene) {
        ...
        if (count <= 0 && !attrs.starfieldCube && !attrs.starfieldData)
            return;
        /*
        ...
        */
        ...
        if (dataRoot && typeof mat0?.setTexture === 'function') {
            ...
            */
            const data = new BABYLON.CubeTexture(dataRoot, scene, ['_px.png', '_py.png', '_pz.png', '_nx.png', '_ny.png', '_nz.png'], true);
        ...
        see.
        */
        const R = (attrs.skyboxSize || 1000) * 0.45;
        for (let i = 0; i < count; i++) {
     ...
     */
    _applyMoons(sm, isDay, sunVector, daylight) {
        if (sm.setArray4 == null)
            return;
        const a = new Array(16).fill(0);
        const b = new Array(16).fill(0);
        const moons = Array.from(this.querySelectorAll('tosi-b3d-moon')).slice(0, 4);
        ...
        moons.forEach((m, i) => {
            ...
            let c;
            try {
                c = BABYLON.Color3.FromHexString(String(m.color || '#dddddd').slice(0, 7));
            }
            catch {
    ...
    _vacuumNow() {
        const attrs = this;
        const full = attrs.spaceFull;
        const start = attrs.spaceStart;
        /*
        ...
        */
        ...
        const dust = Math.min(1, Math.max(0, Number(attrs.dust ?? 0))) * (1 - climbed);
        this._gasNow = gas;
     ...
     */
    _applyVeil() {
        if (!this._forkedSky || this.owner == null || this.mesh == null)
            return;
        const sm = this.mesh.material;
        if (sm?.setFloat == null)
            return;
        ...
        sm.setFloat('b3dTwinkle', Math.max(0, Number(attrs.starfieldTwinkle) || 0) * this._gas);
    }
    updateSky() {
        ...
        */
        ...
        const dustGrey = presence > 0.001 ? Math.max(0, Math.min(1, 1 - this._gas / presence)) : 0;
        if (this._forkedSky)
            material.setFloat?.('b3dDustGrey', dustGrey);
        /*
        ...
        */
        ...
        if (this._starfieldMesh != null) {
            ...
            const m = this._starfieldMesh.material;
            /*
            ...
            */
            const nb = attrs.nebulaBrightness * exposed;
            for (let i = 0; i < this._nebulaMats.length; i++) {
                const base = this._nebulaBase[i];
                this._nebulaMats[i].emissiveColor.set(base.r * nb, base.g * nb, base.b * nb);
            }
        }
        ...
        */
        if (this.owner != null && attrs.starDistance > 0) {
            if (this.starEl == null) {
                this.starEl = this.owner.querySelector('tosi-b3d-star');
            }
    ...
    sceneReady(owner, scene) {
        ...
        */
        const wantsCube = !!this.starfieldCube;
        /*
        ...
        */
        ...
        const material = forked
            ? makeForkedSkyMaterial(scene)
            : new SkyMaterial('skybox', scene);
        this._forkedSky = forked;
        if (!forked) {
            console.warn("b3d-skybox: could not derive a sky shader from Babylon — `starfieldCube` and the medium veil are unavailable. Babylon's sky shader may have changed shape; see registerForkedSky.");
        }
        void wantsCube;
