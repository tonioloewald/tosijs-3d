 ...
 */
export function programPosition(p, on, sinceChange) {
    if (p == null)
        return on ? 1 : null;
    const { a, b } = splits(p);
 ...
 */
export function sampleLight(p, on, sinceChange) {
    if (!isAnimated(p)) {
 ...
 */
export function shiftHue(rgb, deg, satScale = 1) {
    if (!deg && satScale === 1)
        return { ...rgb };
    const { r, g, b } = rgb;
 ...
 */
export function lightProgramSchema(extra = {}) {
    const channel = (title, kind = 'profile') => ({
        ...curveSchema(kind),
        title,
    });
    return {
