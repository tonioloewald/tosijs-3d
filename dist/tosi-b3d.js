        ...
        const frame = base.sessionManager.onXRFrameObservable.add(() => {
            ...
            if (piloted != null) {
                ...
                if (view === 'cockpit') {
                    ...
                    BABYLON.Quaternion.SlerpToRef(followQuat, aimQ, Math.min(1, 20 * dt), followQuat);
    ...
    _attachXrPanel(base) {
        ...
        const frame = base.sessionManager.onXRFrameObservable.add(() => {
            ...
            if (fwd.lengthSquared() < 1e-4)
                fwd.set(0, 0, 1);
            fwd.normalize();
            plane.position.set(hp.x + fwd.x * FORWARD, hp.y + ABOVE, hp.z + fwd.z * FORWARD);
            plane.rotationQuaternion = BABYLON.Quaternion.RotationYawPitchRoll(Math.atan2(-fwd.x, -fwd.z), Math.atan2(ABOVE, FORWARD), 0);
            plane.visibility = 1;
        });
