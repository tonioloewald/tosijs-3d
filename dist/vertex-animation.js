 ...
 */
export function vatLayout(vertexCount, frameCount, maxWidth = 2048) {
 ...
 */
export function vatTexel(layout, vertexIndex, frame) {
    const v = Math.max(0, Math.min(layout.vertexCount - 1, Math.floor(vertexIndex)));
    const f = Math.max(0, Math.min(Math.max(0, layout.frameCount - 1), Math.floor(frame)));
    return {
        x: v % layout.width,
 ...
 */
export function framePair(clip, phase) {
    const frames = Math.max(1, Math.floor(clip.frames));
    if (!Number.isFinite(phase))
        return { a: clip.start, b: clip.start, t: 0 };
    if (frames === 1)
        return { a: clip.start, b: clip.start, t: 0 };
    if (clip.loop === true) {
...
export function phaseAt(clip, seconds, speed = 1) {
 ...
 */
export function framesForClip(durationSeconds, fps) {
 ...
 */
export function vatBytes(layout, textures = 2, channels = 4, bytesPerChannel = 2) {
    return (layout.width *
        layout.height *
        channels *
        bytesPerChannel *
...
export function socketLayout(names, frameCount, texelsPerSocket = 2) {
    return {
        names,
...
export function socketTexel(layout, socket, frame, channel = 0) {
    const i = layout.names.indexOf(socket);
    if (i < 0)
        return null;
    const f = Math.max(0, Math.min(Math.max(0, layout.frameCount - 1), Math.floor(frame)));
    return {
        x: i * layout.texelsPerSocket +
            Math.max(0, Math.min(layout.texelsPerSocket - 1, channel)),
        y: f,
    };
}
//# sourceMappingURL=vertex-animation.js.map