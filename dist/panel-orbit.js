 ...
 */
export function orbitPosition(o) {
 ...
 */
export function orbitOf(p) {
    const radius = Math.hypot(p.x, p.y, p.z);
    if (radius < 1e-9)
        return { azimuthDeg: 0, elevationDeg: 0, radius: 0 };
 ...
 */
export function rubberBand(over, max = ORBIT_RUBBER_BAND) {
    if (!(max > 0) || !Number.isFinite(over))
        return 0;
 ...
 */
export function bandOrbit(o) {
    const soften = (v, lo, hi) => v > hi ? hi + rubberBand(v - hi) : v < lo ? lo + rubberBand(v - lo) : v;
    return {
        azimuthDeg: soften(o.azimuthDeg, -ORBIT_MAX_AZIMUTH, ORBIT_MAX_AZIMUTH),
        elevationDeg: soften(o.elevationDeg, ORBIT_MIN_ELEVATION, ORBIT_MAX_ELEVATION),
        radius: Math.max(0.1, o.radius),
    };
}
/**
 ...
 */
export function orbitFromDirection(dir, current) {
 ...
 */
export function angularHeight(height, radius) {
 ...
 */
export function orbitClampedBy(wanted) {
    const held = clampOrbit(wanted);
    return Math.max(Math.abs(held.azimuthDeg - wanted.azimuthDeg), Math.abs(held.elevationDeg - wanted.elevationDeg));
}
//# sourceMappingURL=panel-orbit.js.map