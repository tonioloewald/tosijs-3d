...
function hexToColor4(hex) {
    const c = BABYLON.Color3.FromHexString(hex.length === 4 || hex.length === 7 ? hex : hex.slice(0, 7));
...
export class B3dParticles extends Component {
    ...
    applySettings() {
        ...
        if (attrs.useSceneGravity) {
            ...
            ps.gravity = new BABYLON.Vector3(attrs.gravityX, attrs.gravityY, attrs.gravityZ);
        }
        // Colors
        ...
        if (attrs.texture) {
            if (!ps.particleTexture ||
                ps.particleTexture.url !== attrs.texture) {
        ...
        if (attrs.emitterShape !== this._currentShape ||
    ...
    applyEmitterShape(ps, attrs) {
        switch (attrs.emitterShape) {
            ...
            case 'cone':
                ...
                break;
            case 'box':
                ps.createBoxEmitter(new BABYLON.Vector3(-0.5, 1, -0.5), new BABYLON.Vector3(0.5, 1, 0.5), new BABYLON.Vector3(-0.5, 0, -0.5), new BABYLON.Vector3(0.5, 0, 0.5));
                break;
            default:
                // point: emit upward by default
                ps.createPointEmitter(new BABYLON.Vector3(-0.1, 1, -0.1), new BABYLON.Vector3(0.1, 1, 0.1));
                break;
        }
    }
}
