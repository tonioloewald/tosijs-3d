...
export class XrFrames {
    ...
    attachInput(input) {
        ...
        const added = (controller) => {
            ...
            const set = () => {
                if (hand === 'left')
                    this.leftGrip = controller.grip ?? controller.pointer;
                else if (hand === 'right')
    ...
    poseHand(frame, grip) {
        ...
        if (arq)
            frame.rotationQuaternion.copyFrom(arq);
    }
    ...
    update(dt) {
        ...
        this.bodyYaw = dampYaw(this.bodyYaw, headYaw, dt, this.bodyYawRate, this.bodyYawDeadband);
        // Body: floor under the head (rig-local), damped torso yaw.
        this.body.position.set(cam.position.x, 0, cam.position.z);
        BABYLON.Quaternion.RotationYawPitchRollToRef(this.bodyYaw, 0, 0, this.body.rotationQuaternion);
