 ...
 */
export function fitDistance(desired, hit, options = {}) {
 ...
 */
export function easeDistance(current, target, dt, options = {}) {
    if (!(dt > 0))
        return current;
    const tau = target < current ? options.inSeconds ?? 0 : options.outSeconds ?? 0.45;
 ...
 */
export function clearOfBand(y, subjectY, band, margin = 0.05) {
 ...
 */
export function forceFirstPerson(hit, wasFirstPerson = false, enter = 0.9, leave = 1.4) {
    if (!(hit > 0) || !Number.isFinite(hit))
        return false;
 ...
 */
export function fitChase(desired, state, input, options = {}) {
    const hit = input.hit ?? Infinity;
    const firstPerson = forceFirstPerson(hit, state.firstPerson ?? false, options.firstPersonAt, options.firstPersonLeave);
...
//# sourceMappingURL=camera-fit.js.map