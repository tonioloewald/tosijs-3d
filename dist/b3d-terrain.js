...
export class B3dTerrain extends B3dChild {
    ...
    makeHeightFn() {
        ...
        const circumU = surfaceType === 'torus'
            ? 2 * Math.PI * attrs.majorRadius
            : 2 * Math.PI * radius;
        const circumV = surfaceType === 'sphere'
            ? Math.PI * radius
            : surfaceType === 'torus'
        ...
        this._debugOff = owner.addDebugSource({
            ...
            lines: () => {
                ...
                return [
                    ...
                    `worst ${n(d.worstFrameMs, 1)}ms · ${d.worstFrameTiles} tiles${d.worstFrameSaturated ? ' (capped)' : ''}`,
                    `tile ${n(d.msPerTile)}ms = field ${n(d.fieldMsPerTile)} + gpu ${n(d.uploadMsPerTile)}`,
                    // movable = the ceiling on what a worker or wasm could EVER take; the GPU
                    // upload cannot move off the main thread at all.
                    `movable ${(d.movableShare * 100).toFixed(0)}% · ${n(d.nsPerSample, 0)}ns/sample · ${d.tiles} tiles`,
                ];
            },
            actions: [
                {
    ...
    generateTileMesh(tile, subdivisions, cell) {
        ...
        buildTileField(this.makeHeightFn(), cell.cx, cell.cz, subdivisions, tileSize, this._fieldScratch, positions, normals, 
    ...
    */
    ...
    _syncBiome() {
        ...
        if (climate !== this._syncedClimate) {
            ...
            if (t >= 0)
                p.baseTemperature = Math.min(1, t);
            if (m >= 0)
                p.mapMoisture = Math.min(1, m);
            if (v >= 0)
                p.volcanicScale = v;
        }
    }
