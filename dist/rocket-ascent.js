 ...
 */
export function rocketAscent(options = {}) {
    ...
    */
    ...
    const stack = ROCKET_STACK.map((part) => b3dProp({
    ...
    const ctrl = b3dController({
        drive: (_input, dt) => {
            ...
            for (let i = 0; i < stack.length; i++) {
                ;
                stack[i].y = y + ROCKET_STACK[i].dy * scale;
            }
            ;
            anchor.y = y + ROCKET_HEIGHT * 0.5 * scale;
            const ex = exhaust;
            ex.y = y;
...
//# sourceMappingURL=rocket-ascent.js.map