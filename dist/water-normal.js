 ...
 */
export function tileHeight(noise, u, v, octaves = 3) {
    let h = 0;
    let amp = 1;
    let freq = 1;
    let norm = 0;
 ...
 */
export function writeNormalMap(rgba, size, height, strength = 2) {
    const step = 1 / size;
 ...
 */
export function waterNormalTexture(scene, size = 256, seed = 1337) {
    const tex = new BABYLON.DynamicTexture('w3d-water-normal', { width: size, height: size }, scene, false);
    const ctx = tex.getContext();
    const img = ctx.createImageData(size, size);
    const noise = new PerlinNoise(seed);
    writeNormalMap(img.data, size, (u, v) => tileHeight(noise, u, v));
...
//# sourceMappingURL=water-normal.js.map