...
export class B3dElevator extends AbstractMesh {
    ...
    _occupied(scene) {
        ...
        */
        ...
        for (const m of scene.meshes) {
            if (m === mesh || !m.isEnabled() || m.getTotalVertices() === 0)
                continue;
            ...
            if (p.y < top - 0.1 || p.y > top + 0.6)
                continue;
            if (Math.abs(p.x - cx) > hw)
                continue;
            if (Math.abs(p.z - cz) > hd)
                continue;
            return true;
        }
        return false;
    ...
    sceneReady(owner, scene) {
        ...
        const attrs = this;
        this.mesh = BABYLON.MeshBuilder.CreateBox(this.meshName, { width: this.width, height: this.thickness, depth: this.depth }, scene);
...
//# sourceMappingURL=b3d-elevator.js.map