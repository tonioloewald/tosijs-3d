 ...
 * ```
 */
export function curveProgram3d(config = {}) {
    let program = { ...(config.value ?? {}) };
    const present = PROGRAM_CHANNELS.filter((c) => program[c] != null);
    const channels = config.channels ??
        (present.length > 0
            ? Array.from(new Set(['brightness', ...present]))
            : ['brightness']);
    ...
    */
    const markers = curveMarkers([program.attackEnd ?? 0.35, program.sustainEnd ?? 0.75], {
        labels: ['attack', 'decay'],
        handleChange: (v) => {
...
//# sourceMappingURL=curve-program.js.map