...
function timeScene(gl, scene, frames = FRAMES) {
...
function buildFillScene(engine) {
...
function buildVertexScene(engine) {
    const scene = new BABYLON.Scene(engine);
    const cam = new BABYLON.FreeCamera('c', new BABYLON.Vector3(0, 0, -40), scene);
    cam.setTarget(BABYLON.Vector3.Zero());
    const ground = BABYLON.MeshBuilder.CreateGround('g', { width: 4, height: 4, subdivisions: VERTEX_SUBDIV }, scene);
...
function buildDrawScene(engine) {
    const scene = new BABYLON.Scene(engine);
...
async function detectImmersiveVr() {
    try {
...
export function hydrateProfileFromCache() {
    ...
    if (stored == null || stored.probeVersion !== PROBE_VERSION)
        return false;
    setPerfProfile(resolveProfile(stored.measurements, {
        cached: true,
...
export class B3dProbe extends Component {
    ...
    connectedCallback() {
        ...
        this.ready = runProbe({ force: this.force, ttlDays: this.ttlDays })
            ...
            .then((profile) => {
            this.dispatchEvent(new CustomEvent('profile', { detail: profile, bubbles: true }));
            return profile;
        });
    }
}
