...
import { parseColor, formatColor, rgbToHsv, hsvToRgb, contrastInk, } from './color.js';
import { handlerOf } from './handler-of.js';
import { w3dTheme } from './w3d-theme.js';
const { g, rect, text, circle, defs, linearGradient, stop, pattern } = svgElements;
let seq = 0;
/**
 * A colour picker.
 *
...
export function color3d(config = {}) {
    ...
    const hueStops = [0, 60, 120, 180, 240, 300, 360].map((h) => stop({
        offset: `${(h / 360) * 100}%`,
        'stop-color': formatColor(hsvToRgb({ h, s: 1, v: 1, a: 1 })),
    }));
    hueGrad.append(...hueStops);
    sqSat.append(stop({ offset: '0%', 'stop-color': '#ffffff' }), stop({ offset: '100%', 'stop-color': '#ff0000' }));
    sqVal.append(stop({ offset: '0%', 'stop-color': '#000000', 'stop-opacity': '0' }), stop({ offset: '100%', 'stop-color': '#000000', 'stop-opacity': '1' }));
    alphaGrad.append(stop({ offset: '0%', 'stop-color': '#ffffff', 'stop-opacity': '1' }), stop({ offset: '100%', 'stop-color': '#ffffff', 'stop-opacity': '0' }));
    checks.append(rect({ x: 0, y: 0, width: 8, height: 8, fill: '#8a8f98' }), rect({ x: 0, y: 0, width: 4, height: 4, fill: '#c9ced6' }), rect({ x: 4, y: 4, width: 4, height: 4, fill: '#c9ced6' }));
    const sqHue = rect({ rx: 4, fill: `url(#${uid}-s)` });
    const sqShade = rect({ rx: 4, fill: `url(#${uid}-v)` });
    el.append(defs({}, sqSat, sqVal, hueGrad, alphaGrad, checks), ...(cap ? [cap] : []), sq, sqHue, sqShade, sqDot, hueStrip, hueDot, ...(showAlpha ? [alphaBack, alphaStrip, alphaDot] : []), swatchBack, swatch, readout);
    // --- geometry --------------------------------------------------------------
    const STRIP = 16;
    const GAP = 8;
    const SWATCH = 26;
    ...
    const emit = (commit) => {
        ...
        handlerOf(config, 'handleChange', 'onChange')?.(hex);
        if (commit)
            config.handleCommit?.(hex);
    };
    const paint = () => {
        const rgb = hsvToRgb(hsv);
        const hex = formatColor(rgb);
        const pure = formatColor(hsvToRgb({ h: hsv.h, s: 1, v: 1, a: 1 }));
        sqSat.lastChild?.setAttribute('stop-color', pure);
        alphaGrad.firstChild?.setAttribute('stop-color', pure);
        ...
        if (showAlpha) {
            ...
            alphaDot.setAttribute('y', String(alphaBox.y + (1 - hsv.a) * alphaBox.h - 1.5));
        }
    };
    const api = {
        el,
        ...
        setValue(hex) {
            ...
            if (rgb == null)
                return; // null, not black — see `parseColor`
            hsv = rgbToHsv(rgb);
            paint();
        },
        layout(width) {
        ...
        hitTest(x, y) {
            const inSq = x >= box.x && x <= box.x + box.w && y >= box.y && y <= box.y + box.h;
            const inHue = x >= hueBox.x - 3 &&
        ...
        handle(kind, x, y) {
            ...
            */
            ...
            if (grabbed == null)
                return;
            const f = (v, lo, span) => Math.max(0, Math.min(1, (v - lo) / (span || 1)));
            if (grabbed === 'square') {
