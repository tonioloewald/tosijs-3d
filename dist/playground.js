...
export function playground(options = {}) {
    ...
    if (options.lighting !== false) {
        parts.push(b3dSkybox({ timeOfDay: options.timeOfDay ?? 9 }), 
        /*
    ...
    */
     ...
     */
    const wall = (name, x, z, w, h, d, color = STONE, y = h / 2) => b3dBox({
        meshName: name,
        x,
        y,
        z,
    ...
    parts.push(wall('lowwall', -18, line, 11, 1.1, 0.6), wall('pillar-a', -4, line, 1.2, 2.4, 1.2, DARK_STONE), wall('pillar-b', 0, line, 1.2, 2.4, 1.2, DARK_STONE), 
    // The bunker: two walls meeting, so the shelter is off to one side of the
    // threat rather than facing it.
    wall('bunker-a', 12, line - 2.5, 0.6, 2.2, 6, DARK_STONE), wall('bunker-b', 15, line + 0.2, 6, 2.2, 0.6, DARK_STONE));
    /*
    ...
    */
    ...
    wall('rail-top', 24, line, 7, 0.22, 0.3, METAL, 1.1), wall('rail-post-a', 21, line, 0.18, 1.1, 0.18, METAL), wall('rail-post-b', 24, line, 0.18, 1.1, 0.18, METAL), wall('rail-post-c', 27, line, 0.18, 1.1, 0.18, METAL));
    /*
    ...
    */
    parts.push(wall('crate-step', -14, 10, 1.6, 0.4, 1.6, CRATE), wall('crate-mantle', -12, 12.5, 1.6, 0.9, 1.6, CRATE), wall('crate-high', -10, 15, 1.6, 1.6, 1.6, CRATE), 
    // …and one stack that gets you onto the catwalk, if you take them in order.
    wall('crate-stair-a', 6, 14, 2, 0.5, 2, CRATE), wall('crate-stair-b', 8.5, 15.5, 2, 1.1, 2, CRATE), 
    ...
    */
    parts.push(wall('catwalk-deck', 18, 17, 12, 0.4, 3.5, METAL, 2.4), wall('catwalk-rail', 18, 15.45, 12, 0.14, 0.14, METAL, 3.35), wall('catwalk-midrail', 18, 15.45, 12, 0.1, 0.1, METAL, 2.98));
    /*
    ...
    */
    for (let i = 0; i <= 6; i++) {
        parts.push(wall(`catwalk-post-${i}`, 12.3 + i * 1.9, 15.45, 0.12, 0.75, 0.12, METAL, 2.98));
    }
    /*
    ...
    */
    ...
    parts.push(wall('tower', towerX, towerZ, 6, towerH, 6, DARK_STONE), b3dBox({
    ...
    */
    const callSwitch = (name, x, z, base) => {
        parts.push(wall(`${name}-post`, x, z, 0.18, 1.1, 0.18, METAL, base + 0.55), wall(`${name}-head`, x, z, 0.42, 0.42, 0.2, '#d8b24a', base + 1.15), b3dInteractive({
            target: `${name}-head`,
            reach: 2.5,
            highlight: '#ffdd66',
            whenActivated: () => {
                const lift = document.querySelector('tosi-b3d-elevator[mesh-name="lift-switch"]');
                lift?.call?.();
            },
        }));
    };
