...
export class B3dPatch extends B3dChild {
    ...
    sceneReady(owner, scene) {
        this._terrain = owner.querySelector('tosi-b3d-terrain');
        if (this._terrain == null) {
            console.warn('b3d-patch: no <tosi-b3d-terrain> in the scene — nothing to cut');
            return;
        }
        if (this.material == null)
            this.material = this._wallMaterial(scene);
        this._register();
    ...
    _register() {
        ...
        terrain.patches = [
            ...terrain.patches.filter((p) => p.__patch !== this),
            {
                minX: this.minX,
                maxX: this.maxX,
                minZ: this.minZ,
        ...
        terrain.patchMask = (x, z) => {
            if (previous?.(x, z))
                return true; // patches compose; never clobber a sibling
            if (!this._resident)
                return false; // sealed at distance
    ...
    _wallMaterial(scene) {
        ...
        const terrainPlugin = this._terrain?.biomePlugin;
     ...
     */
    _cavity = (x, y, z) => {
        if (!this._resident || this._height == null)
            return false;
    ...
    _update(owner) {
        ...
        const camX = cam.globalPosition.x + terrain.originOffsetX;
        const camZ = cam.globalPosition.z + terrain.originOffsetZ;
        const resident = patchResident((this.minX + this.maxX) / 2, (this.minZ + this.maxZ) / 2, this.level, camX, camZ, cfg);
    ...
    _measureY() {
        ...
        for (let i = 0; i <= N; i++) {
            for (let j = 0; j <= N; j++) {
                ...
                if (this._footprintAt(x, z) > this.spacing * 2)
                    continue;
                const y = h(x, z);
                if (y < lo)
                    lo = y;
    ...
    _planChunks() {
        ...
        if (cam != null) {
            ...
            this._pending.sort((a, b) => {
                const da = (a.ix * step - cx) ** 2 +
                    (a.iy * step - cy) ** 2 +
                    (a.iz * step - cz) ** 2;
                const db = (b.ix * step - cx) ** 2 +
                    (b.iy * step - cy) ** 2 +
                    (b.iz * step - cz) ** 2;
                return da - db;
            });
     ...
     */
    _chunkCouldHaveSurface(field, ox, oy, oz, size) {
        const half = size / 2;
        const cx = ox + half;
        const cy = oy + half;
        const cz = oz + half;
        ...
        if (!allNeg && !allPos)
            return true; // a sign change: definitely a surface
        return nearest <= half * Math.sqrt(3);
    }
    _extractOne(owner, ix, iy, iz) {
        const n = Math.max(2, Math.floor(this.chunkCells));
        const lattice = {
        ...
        const field = (x, y, z) => this.field ? this.field(x, y, z, base(x, y, z)) : base(x, y, z);
        ...
        const clip = (x, y, z) => height == null || y < height(x, z) - surfaceMargin;
        const key0 = `${ix},${iy},${iz}`;
        const size = n * this.spacing;
        if (!this._chunkCouldHaveSurface(field, ix * n * this.spacing, iy * n * this.spacing, iz * n * this.spacing, size)) {
            this._chunks.set(key0, null); // remember the miss
            return;
        }
        const mesh = extractChunk(field, { ix: ix * n, iy: iy * n, iz: iz * n, nx: n, ny: n, nz: n }, { ...lattice, clip });
        const key = `${ix},${iy},${iz}`;
        if (mesh.triangleCount === 0) {
            this._chunks.set(key, null); // remember the miss
            return;
        }
        ...
        m.position.set(-this._terrain.originOffsetX, 0, -this._terrain.originOffsetZ);
    ...
    sceneDispose() {
        ...
        const terrain = this._terrain;
        if (terrain != null) {
            terrain.patches = terrain.patches.filter((p) => p.__patch !== this);
        }
        this._terrain = null;
        super.sceneDispose();
    }
...
//# sourceMappingURL=b3d-patch.js.map