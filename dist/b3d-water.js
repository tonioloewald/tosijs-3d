...
export class B3dWater extends AbstractMesh {
    ...
    updateWater() {
        ...
        this.waterMaterial.windDirection = new BABYLON.Vector2(wind.windDirectionX, wind.windDirectionY);
    ...
    sceneReady(owner, scene) {
        ...
        */
        this._windTick = () => {
            ...
            if (Math.abs(mat.windForce - next.windForce) < 1e-4 &&
     ...
     */
    _fogTheSky(submerged) {
        if (submerged === this._skyFogged)
            return;
    ...
    _skyMeshes() {
        ...
        for (const m of scene.meshes) {
            if (m.infiniteDistance && /skybox|skydome/i.test(m.name))
                out.push(m);
        }
        return out;
    }
    ...
    _applyFollow() {
        ...
        if (!cam || !this.mesh)
            return;
        const size = Math.max(1, this.waterSize);
        ...
        if (this.mesh.position.x === sx && this.mesh.position.z === sz)
            return;
        ...
        const bump = this.waterMaterial?.bumpTexture;
        if (bump) {
