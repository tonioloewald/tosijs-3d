...
export class DestroyableBehavior {
    ...
    _obs;
    _capacity;
    constructor(owner, host, spec, death = {}) {
        this.owner = owner;
        this.host = host;
        this.spec = spec;
        this.death = death;
    ...
    _flash(hp) {
        ...
        if (mesh == null)
            return;
        const frac = this._capacity > 0 ? Math.max(0, Math.min(1, hp / this._capacity)) : 1;
    ...
    _die() {
        ...
        if (mesh != null) {
            if (d.explode) {
                ...
                try {
                    explodeMesh(mesh, scene, {
                        force: d.explodeForce ?? 6,
                        disposeOriginal: true,
                    });
                }
                catch (err) {
                    console.warn('destroyable: explode failed; removing the mesh instead', err);
                    mesh.dispose();
                }
            }
...
//# sourceMappingURL=destroyable-behavior.js.map