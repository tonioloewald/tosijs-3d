...
export class B3dBlackHole extends Component {
    ...
    registerShaders() {
        // Only register once
        if (BABYLON.Effect.ShadersStore['accretionDiskVertexShader'])
            return;
    ...
    buildHorizon() {
        ...
        const mesh = BABYLON.MeshBuilder.CreateSphere('event-horizon', { diameter: radius * 2, segments }, this.owner.scene);
    ...
    buildSurfaceGlow() {
        ...
        const segments = Math.max(8, Math.round(attrs.subdivisions / 2));
        const mesh = BABYLON.MeshBuilder.CreateSphere('horizon-glow', { diameter: glowRadius * 2, segments }, this.owner.scene);
        // Rim glow: bright at silhouette edges, transparent face-on
        const mat = new BABYLON.ShaderMaterial('horizon-glow-mat', this.owner.scene, { vertex: 'horizonGlow', fragment: 'horizonGlow' }, {
    ...
    buildAnnulusMesh(name, innerR, outerR) {
        const attrs = this;
        const subs = attrs.subdivisions;
    ...
    createDiskMaterial(name) {
        const attrs = this;
        const mat = new BABYLON.ShaderMaterial(name, this.owner.scene, { vertex: 'accretionDisk', fragment: 'accretionDisk' }, {
    ...
    buildPhotonRing() {
        ...
        const mesh = BABYLON.MeshBuilder.CreateTorus('photon-ring', {
            diameter: ringRadius * 2,
            thickness: tubeRadius * 2,
            tessellation,
        }, this.owner.scene);
        const mat = new BABYLON.ShaderMaterial('photon-ring-mat', this.owner.scene, { vertex: 'photonRing', fragment: 'photonRing' }, {
            attributes: ['position', 'normal'],
            uniforms: [
                'worldViewProjection',
                'world',
    ...
    updateOptions() {
        ...
        if (this.diskMesh?.material) {
            const mat = this.diskMesh.material;
            mat.setFloat('brightness', attrs.diskBrightness);
