 ...
 */
export class B3dLamp extends B3dChild {
    ...
    */
    ...
    buildFixture(scene) {
        if (this.url) {
            ...
            BABYLON.LoadAssetContainerAsync(this.url, scene)
                ...
                .catch((err) => console.warn(`b3d-lamp: could not load ${this.url}`, err));
            return;
        }
        if (isOff(this.geometry))
            return;
    ...
    setupShadows() {
        ...
        if (!(light instanceof BABYLON.ShadowLight)) {
            warnOnce(`b3d-lamp: ${this.tagName.toLowerCase()} cannot cast shadows — Babylon's RectAreaLight is not a ShadowLight. Ignoring shadows="on".`);
            return;
        }
    ...
    render() {
        super.render();
        if (this.light == null || this.node == null)
            return;
        this.node.position.set(this.x, this.y, this.z);
        this.baseIntensity = this.intensity;
        this.baseRange = this.range;
        ...
        if (!isAnimated(this.program)) {
            ...
            const ranged = this.light;
            if (typeof ranged.range === 'number')
                ranged.range = this.range;
        }
        this.positionLight();
    }
    ...
    positionLight() {
        const l = this.light;
        if (l?.position != null)
            l.position.set(this.x, this.y, this.z);
    }
    sceneDispose() {
        if (this._tick != null) {
            this._tick.remove();
 ...
 */
function sceneDeltaSeconds(scene) {
...
export class B3dPointLight extends B3dLamp {
    ...
    createLight(scene) {
        const light = new BABYLON.PointLight('point-light', new BABYLON.Vector3(this.x, this.y, this.z), scene);
    ...
    createGeometry(scene) {
        const bulb = BABYLON.MeshBuilder.CreateSphere('lamp-bulb', { diameter: 0.25 * this.geometryScale, segments: 12 }, scene);
...
export class B3dSpotLight extends B3dLamp {
    ...
    createGeometry(scene) {
        ...
        const housing = BABYLON.MeshBuilder.CreateCylinder('lamp-housing', {
            diameterTop: 0.34 * s,
        ...
        if (dir.lengthSquared() > 0) {
            ...
            const angle = Math.acos(Math.min(1, Math.max(-1, BABYLON.Vector3.Dot(BABYLON.Axis.Y, dir))));
...
export class B3dAreaLight extends B3dLamp {
    ...
    createLight(scene) {
        const light = new BABYLON.RectAreaLight('area-light', new BABYLON.Vector3(this.x, this.y, this.z), this.width, this.height, scene);
        light.intensity = this.intensity;
        light.diffuse = BABYLON.Color3.FromHexString(this.diffuse);
        return light;
    }
    createGeometry(scene) {
        ...
        const panel = BABYLON.MeshBuilder.CreatePlane('lamp-panel', {
            width: this.width,
            height: this.height,
...
//# sourceMappingURL=b3d-lamp.js.map