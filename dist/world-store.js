...
import { proximityRung, routePortals, containmentPath, rungNominal, } from './world-topology.js';
...
export class WorldStore {
    ...
    moveEntity(id, position) {
        ...
        const inside = this.occupancy.get(id) ?? new Set();
    ...
    interact(actorId, targetId) {
        ...
        if (!target)
            return;
        if (target.components.interactable?.locked)
            return;
        target.lastInteractedAt = this.state.now;
        this.emit({ type: 'interaction', at: this.state.now, actorId, targetId });
    }
    chooseConversation(actorId, targetId, optionId) {
        if (!this.state.entities[targetId])
            return;
        this.emit({
    ...
    applyDamage(targetId, amount, killerId) {
        const target = this.state.entities[targetId];
