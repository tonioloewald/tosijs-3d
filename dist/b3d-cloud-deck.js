...
export class B3dCloudDeck extends B3dChild {
    ...
    */
    ...
    _tint = new BABYLON.Color3(1, 1, 1);
    _borrowed = new Map();
    _onAddition = (a) => {
        for (const m of a.meshes ?? [])
            this._maybeReceive(m);
    };
    /**
    ...
    _maybeReceive(m) {
        ...
        if (map == null || mat == null || !m.receiveShadows)
            return;
        if (m === this.mesh)
            return;
        const kind = mat.getClassName();
        if (kind !== 'StandardMaterial' && !kind.includes('PBR'))
            return;
        map.attachTo(mat);
    }
    _immersion = 0;
    ...
    sceneReady(owner, scene) {
        ...
        const mesh = BABYLON.MeshBuilder.CreateGround('cloud-deck_nocast', {
            width: attrs.size,
            height: attrs.size,
        ...
        */
        ...
        colors.fill(0);
        for (let v = 3; v < colors.length; v += 4)
            colors[v] = 1;
        mesh.setVerticesData(BABYLON.VertexBuffer.ColorKind, colors, true);
        const mat = new BABYLON.ShaderMaterial('cloud-deck', scene, { vertexSource: DECK_VERT, fragmentSource: DECK_FRAG }, {
        ...
        */
        const top = BABYLON.MeshBuilder.CreateGround('cloud-deck-top_nocast', {
            width: attrs.size,
            height: attrs.size,
            subdivisions: Math.max(1, Math.floor(attrs.subdivisions)),
        }, scene);
        ...
        */
        ...
        for (let v = 1; v < topColors.length; v += 4)
            topColors[v] = 1;
        ...
        */
        ...
        if (!isOff(attrs.shadows))
            this._setupShadows(owner, scene);
    }
    ...
    _sync() {
        ...
        */
        ...
        mat.setVector4('drift', new BABYLON.Vector4(dx, dz, dx * 1.9, dz * 1.9 + t * attrs.wind * 0.35));
        ...
        if (scene != null) {
            ...
            mat.setVector4('fogInfos', new BABYLON.Vector4(scene.fogMode, scene.fogStart, scene.fogEnd, scene.fogDensity));
            const cam = scene.activeCamera;
            if (cam != null)
                mat.setVector3('camPos', cam.globalPosition);
        }
        this._immersion = this._immersionAt(this.owner?.scene?.activeCamera?.globalPosition);
        const sun = this.owner?.scene?.lights?.find((l) => l.direction != null);
        mat.setVector3('sunDir', sun?.direction ?? new BABYLON.Vector3(-0.4, -1, -0.3));
        /*
        ...
        */
        ...
        this._tint.set(Math.min(1.15, tint.r * level + 0.12), Math.min(1.15, tint.g * level + 0.12), Math.min(1.15, tint.b * level + 0.13));
    ...
    _bakeField(scene) {
        ...
        */
        ...
        for (let i = 0; i < field.length; i++)
            bytes[i] = Math.round(field[i] * 255);
        const tex = new BABYLON.RawTexture(bytes, size, size, BABYLON.Constants.TEXTUREFORMAT_R, scene, 
    ...
    _bakeWeather(force = false) {
        ...
        */
        ...
        if (positions == null || colors == null || topColors == null)
            return;
    ...
    _bindWeather(mat) {
        ...
        const mesh = this.mesh;
        if (tex == null || mesh == null)
            return;
        mat.setTexture('weatherTex', tex);
        mat.setVector4('weatherWindow', new BABYLON.Vector4(mesh.position.x, mesh.position.z, 1 / Math.max(1, this.size), 0));
    ...
    _fogAt(p) {
        ...
        */
        ...
        const near = p != null && p.y > this.altitude + rise
            ? 0
        ...
        */
        ...
        const darkest = BABYLON.Color3.Lerp(under, top, Math.min(1, this.resolvedTransmission * 0.6));
    ...
    _syncShadows() {
        ...
        */
        ...
        tex.setVector4('drift', new BABYLON.Vector4(this._driftX, this._driftZ, this._driftX2, this._driftZ2));
        /*
    ...
    _applyGloom(sun) {
        ...
        const below = cam != null && cam.y < this.altitude + this.topRise + this._halfDepth();
        ...
        */
        const num = (v, fallback) => Number.isFinite(v) ? Math.max(0, Math.min(1, v)) : fallback;
        /** How far past `threshold` the transmission has fallen, smoothed, 0…1. */
        const ramp = (threshold) => {
            if (!below || threshold <= 0 || t >= threshold)
                return 0;
        ...
        */
        ...
        for (const light of [...this._borrowed.keys()]) {
            if (scene.lights.includes(light))
                continue;
    ...
    _releaseLights() {
        for (const [light, rec] of this._borrowed)
            light.intensity = rec.base;
...
//# sourceMappingURL=b3d-cloud-deck.js.map