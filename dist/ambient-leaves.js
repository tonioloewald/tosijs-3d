 ...
 */
function leafTexture(scene) {
    const existing = scene.getTextureByName?.('ambient-leaf');
    if (existing)
        return existing;
    const s = 64;
    const tex = new BABYLON.DynamicTexture('ambient-leaf', { width: s, height: s }, scene, false);
...
export class LeafField {
    ...
    _targetOn = 0;
    _live = 0;
    _dq = new BABYLON.Quaternion();
    constructor(scene, opts) {
        this._radius = opts.radius;
        this._near = opts.near;
        const cap = Math.max(1, opts.capacity);
        const tmpl = BABYLON.MeshBuilder.CreatePlane('leaf-template', { size: 1, sideOrientation: BABYLON.Mesh.DOUBLESIDE }, scene);
        ...
        for (const layer of scene.effectLayers ?? []) {
            if (layer.getClassName() === 'GlowLayer') {
                ;
                layer.addExcludedMesh(this._mesh);
            }
        }
        ...
        sps.initParticles = () => {
            for (const p of sps.particles) {
                ...
                p.rotationQuaternion = BABYLON.Quaternion.FromEulerAngles(Math.random() * TAU, Math.random() * TAU, Math.random() * TAU);
                p.props.scale = 0;
                p.scaling.setAll(0);
                this._place(p, true);
            }
    ...
    update(dt, target) {
        ...
        this._targetOn = Math.round(Math.min(this._sps.nbParticles, Math.max(0, target)));
        this._live = 0;
    ...
    _updateParticle(p) {
        ...
        // from a stale spot.
        if (on && !pr.wasOn) {
            const size = [pr.baseScale, pr.baseScale];
...
//# sourceMappingURL=ambient-leaves.js.map