...
export class B3dSvgPlane extends AbstractMesh {
    ...
    _installWorldDialog(scene) {
        ...
        */
        ...
        const desired = this.z || 2;
        const place = (cam, immediate) => {
            const eye = cam.globalPosition;
            const fwd = cam.getDirection(BABYLON.Vector3.Forward());
            const clearances = [];
            const dirs = [];
            for (const yawDeg of YAW_CANDIDATES) {
                const q = BABYLON.Quaternion.RotationAxis(BABYLON.Vector3.Up(), (yawDeg * Math.PI) / 180);
                const dir = BABYLON.Vector3.Zero();
                fwd.rotateByQuaternionToRef(q, dir);
                dir.normalize();
                dirs.push(dir);
                const hit = scene.pickWithRay(new BABYLON.Ray(eye, dir, desired + 1), 
            ...
            const dist = placementDistance(i >= 0 ? clearances[i] : 0, desired, 0.6, 0.25);
        ...
        this._dialogObs = scene.onBeforeRenderObservable.add(() => {
            ...
            const off = gazeOffAxisDeg(cam.globalPosition, cam.getDirection(BABYLON.Vector3.Forward()), mesh.position);
    ...
    _meshName() {
        const attrs = this;
        let name = 'svg-plane';
        if (attrs.castShadow !== true)
            name += '_nocast';
        if (attrs.receiveShadows !== true)
            name += '_noshadow';
        return name;
    }
    sceneReady(owner, scene) {
        ...
        if (attrs.cornerRadius > 0) {
            ...
            this.mesh = BABYLON.MeshBuilder.CreatePlane(this._meshName(), {
                width: attrs.width,
                height: attrs.height,
        ...
        */
        if (attrs.cameraRelative) {
            if (attrs.placement === 'world')
                this._installWorldDialog(scene);
            else
    ...
    _applyChannel(mat, channel, opaque = false) {
        if (!this._svgTexture)
            return;
    ...
    _attachPointerObserver(scene) {
        ...
        this._pointerObserver = scene.onPointerObservable.add((pointerInfo) => {
            const kind = pointerInfo.type === POINTERDOWN
                ? 'down'
                : pointerInfo.type === POINTERUP
                    ? 'up'
            ...
            const handle = svgEl && typeof svgEl.handlePointer === 'function'
                ? svgEl.handlePointer
                : null;
            const pick = pointerInfo.pickInfo;
            ...
            if (handle) {
                ...
                if (kind === 'down') {
                    ...
                    if (this._pressing)
                        handle('move', this._lastSvgX, this._lastSvgY);
                }
                else {
                    if (this._pressing)
                        handle('up', this._lastSvgX, this._lastSvgY);
    ...
    _dispatchSyntheticEvent(pointerInfo, uvs) {
        const svgEl = this.svgElement;
        if (!svgEl)
            return;
        ...
        const type = pointerInfo.type === POINTERDOWN
            ? 'pointerdown'
            : pointerInfo.type === POINTERUP
                ? 'pointerup'
                : 'pointermove';
        const nativeEvt = pointerInfo.event;
        ...
        target.dispatchEvent(new PointerEvent(type, {
            ...
            */
            ...
            pointerType: nativeEvt.pointerType ?? 'mouse',
...
export function uvToViewBox(uv, vbW, vbH) {
 ...
 */
export function planeLocalToViewBox(local, planeW, planeH, vbW, vbH) {
    return {
 ...
 */
export function panelGesture(active, ev) {
    const actions = [];
...
export function panelScene(opts) {
    ...
    const sceneCreated = (el) => {
        ...
        */
        ...
        const catcher = BABYLON.MeshBuilder.CreatePlane('panel-catcher', { width: width * 3, height: planeH * 3 }, el.scene);
        catcher.visibility = 0;
        catcher.isPickable = false;
        let gesture = null;
        const T = BABYLON.PointerEventTypes;
        const claims = opts.claim ?? opts.target.interactiveAt?.bind(opts.target) ?? (() => true);
        let active = false;
        el.scene.onPointerObservable.add((pi) => {
            ...
            if (active && (kind === 'move' || kind === 'up')) {
                ...
                if (hit && hit.hit && hit.pickedPoint && gesture) {
                    const local = BABYLON.Vector3.TransformCoordinates(hit.pickedPoint, gesture.inv);
                    ev.catcher = planeLocalToViewBox(local, width, planeH, gesture.vbW, gesture.vbH);
                }
                else {
                    ev.catcher = null;
                }
                ...
                if (kind === 'down' && onPlane)
                    ev.claims = claims(ev.x, ev.y);
            }
            const step = panelGesture(active, ev);
            active = step.active;
            for (const a of step.actions) {
                if (a.do === 'route')
                    opts.target.handlePointer(a.kind, a.x, a.y);
                else if (a.do === 'begin')
                    beginGesture();
                else
                    endGesture();
