...
import { boneMask, findBone, UPPER_BODY_ROOTS, } from './bone-mask.js';
/** A skeleton as the pure model wants it: names and parents. */
export function boneHierarchy(skeleton) {
...
const tierKey = (w) => Math.round(w * 100) / 100;
/**
 ...
 */
function tiersFor(scene, source, weightOf, label) {
    ...
    for (const ta of source.targetedAnimations) {
        ...
        */
        const name = (ta.target?.name ?? '').replace(/^Clone of /, '');
    ...
    for (const [weight, animations] of byTier) {
        const group = new BABYLON.AnimationGroup(`${source.name}-${label}-${weight}`, scene);
 ...
 */
export function layerGroups(scene, base, layer, mask) {
    const weightOf = (name) => mask.get(name) ?? 0;
 ...
 */
export function layerOnUpperBody(scene, skeleton, base, layer, options = {}) {
    const bones = boneHierarchy(skeleton);
    const root = findBone(bones, options.roots ?? UPPER_BODY_ROOTS);
    if (root == null)
        return null;
    return layerGroups(scene, base, layer, boneMask(bones, { root, falloff: options.falloff ?? 2 }));
}
//# sourceMappingURL=animation-layers.js.map