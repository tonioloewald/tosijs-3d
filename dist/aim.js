 ...
 */
export const DEFAULT_AIM_LIMITS = {
    yawFree: 45,
    yawMax: 90,
    pitchUp: 60,
    pitchDown: 75,
...
export function clampAim2(aim, limits = DEFAULT_AIM_LIMITS) {
    const pitch = aim.pitchDeg;
    return {
        yawDeg: clampMag(wrapDeg(aim.yawDeg), limits.yawMax),
        pitchDeg: pitch < -Math.abs(limits.pitchUp)
 ...
 */
export function aimToward(bodyYawDeg, dir, limits = DEFAULT_AIM_LIMITS) {
    const len = Math.hypot(dir.x, dir.y, dir.z);
    if (!(len > 1e-9))
        return { yawDeg: 0, pitchDeg: 0 };
    const yaw = (Math.atan2(dir.x, dir.z) * 180) / Math.PI;
    const pitch = (Math.asin(-clampMag(dir.y / len, 1)) * 180) / Math.PI;
    return clampAim2({ yawDeg: wrapDeg(yaw - bodyYawDeg), pitchDeg: pitch }, limits);
}
/** The world direction an aim points, given the body's facing. */
export function aimDirection(bodyYawDeg, aim) {
 ...
 */
export function stepAim(current, target, dt, options = {}) {
    const limits = options.limits ?? DEFAULT_AIM_LIMITS;
    if (!(dt > 0))
        return clampAim2(current, limits);
 ...
 */
export function bodyCatchUp(yawDeg, dt, rateDeg = 180, limits = DEFAULT_AIM_LIMITS) {
    ...
    if (!(dt > 0) || mag <= free)
        return { bodyTurnDeg: 0, yawDeg: twist };
 ...
 * every call site.
 */
export function relaxAim(current, dt, slewDeg = 120, limits = DEFAULT_AIM_LIMITS) {
 ...
 */
export function aimPoseWeights(pitchDeg, limits = DEFAULT_AIM_LIMITS) {
 ...
 */
export function aimAuthority(aim, limits = DEFAULT_AIM_LIMITS) {
    const yaw = Math.abs(wrapDeg(aim.yawDeg)) / Math.max(1e-6, Math.abs(limits.yawMax));
    const span = aim.pitchDeg < 0 ? Math.abs(limits.pitchUp) : Math.abs(limits.pitchDown);
 ...
 */
export function aimWobble(seconds, amplitudeDeg, hz = 0.7, phase = 0) {
    const a = Math.abs(amplitudeDeg);
    if (!(a > 0) || !Number.isFinite(seconds))
        return { yawDeg: 0, pitchDeg: 0 };
