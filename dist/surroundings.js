...
export function bearingIndex(surr, deg) {
...
export function setSample(surr, bearing, heightIndex, distance) {
...
export function sampleAt(surr, bearingIdx, heightIndex) {
    const n = surr.bearings.length;
 ...
 * a bucket boundary.
 */
export function shelterFrom(surr, threatDeg, options = {}) {
    ...
    let best = { height: 0, distance: Infinity, bearing: threatDeg };
 ...
 */
function columnTop(surr, bearingIdx, maxRange) {
    let top = 0;
    for (let h = 0; h < surr.heights.length; h++) {
        if (sampleAt(surr, bearingIdx, h) > maxRange)
            break;
        top = surr.heights[h];
    }
    return top;
}
const bearingOf = (surr, i) => {
    const n = surr.bearings.length;
 ...
 */
export function stanceFor(shelterHeight, stature = {}) {
    const standing = stature.standing ?? 1.8;
    const crouched = stature.crouched ?? standing * 0.55;
    const needed = stature.needed ?? 0.8;
    if (shelterHeight >= standing * needed)
        return 'standing';
    if (shelterHeight >= crouched * needed)
        return 'crouched';
    return null;
}
/**
 ...
 */
export function exposure(shelterHeight, stature = {}, stance = 'standing') {
    const standing = stature.standing ?? 1.8;
    const height = stance === 'crouched' ? stature.crouched ?? standing * 0.55 : standing;
    if (!(height > 0))
        return 1;
 ...
 */
export function inShelter(masked, wasInShelter = false, enter = 0.6, leave = 0.45) {
 ...
 */
export function peekSide(surr, threatDeg, options = {}) {
    const maxRange = options.maxRange ?? 1.2;
    const step = 360 / surr.bearings.length;
    ...
    */
    if (columnTop(surr, centre, maxRange) <= 0)
        return null;
    const clear = (k) => {
        // Clear at chest height is what matters — you lean, you do not crawl.
        let blocked = 0;
        for (let h = 0; h < surr.heights.length; h++) {
            if (surr.heights[h] < (options.minHeight ?? 1.0))
                continue;
            if (sampleAt(surr, centre + k, h) <= maxRange)
                blocked++;
        }
        return blocked === 0;
    };
 ...
 */
export function muzzleClearance(surr, aimDeg, muzzleHeight, options = {}) {
    const maxRange = options.maxRange ?? 1.2;
    const b = bearingIndex(surr, aimDeg);
    const blocked = (h) => sampleAt(surr, b, h) <= maxRange;
    /*
    ...
    */
    let at = -1;
    for (let h = 0; h < surr.heights.length; h++) {
        if (surr.heights[h] <= muzzleHeight)
            at = h;
    }
 ...
 */
export function shuffleToward(surr, threatDeg, options = {}) {
    const maxRange = options.maxRange ?? 1.2;
