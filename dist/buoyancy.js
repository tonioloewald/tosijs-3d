 ...
 */
export function submergedFraction(feetY, height, surfaceY) {
    if (height <= 0)
        return feetY <= surfaceY ? 1 : 0;
 ...
 */
export function buoyantStep(vy, submerged, dt, params = {}) {
    const { gravity = -9.81, buoyancy = 1.15, waterDrag = 4, airDrag = 0.02, thrust = 0, } = params;
    if (dt <= 0)
        return vy;
 ...
 */
export function equilibriumSubmersion(buoyancy = 1.15) {
 ...
 */
export function isSwimming(submerged, restingOnFloor, wasSwimming = false) {
    if (restingOnFloor)
        return false;
    /*
 ...
 */
export function swimBuoyancy(headDepth, params = {}) {
