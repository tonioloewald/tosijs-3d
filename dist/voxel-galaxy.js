...
import { sampleSpiral, starDetailFor, starNameFor, spiralParams, generateNebulae, generateShell, generateStarSystem, GALAXY_DEFAULTS, } from './galaxy-data.js';
import { SPECTRAL_CLASSES, SPECTRAL_WEIGHTS } from './spectral-classes.js';
import { bestHIOf, isInteresting, interestingMix, boringMix, interestingShare, } from './star-populations.js';
const weightOf = (cls) => SPECTRAL_WEIGHTS[SPECTRAL_CLASSES.indexOf(cls)];
/**
 ...
 */
export const DEFAULT_BRIGHT_MIX = [
    ...['O', 'B', 'A', 'F'].map((c) => ({
        spectralClass: c,
        minIndex: 0,
        maxIndex: 9,
 ...
 * smaller budget).
 */
export function starAddress(seed, population, voxel, n) {
    return `${seed}:${population}:${voxel}:${n}`;
}
/** The parts of a star address, or `null` if it is not one. */
export function parseStarAddress(id) {
...
export function voxelGalaxy(options) {
    const o = {
        brightBudget: 5000,
        dimBudget: 95000,
        samples: 100000,
    ...
    */
    ...
    const densityAt = (p) => {
        ...
        const ty = fy - y0;
        const tz = fz - z0;
        const d = (x, y, z) => density[at(Math.min(nx - 1, Math.max(0, x)), Math.min(ny - 1, Math.max(0, y)), Math.min(nz - 1, Math.max(0, z)))];
        const lerp = (a, b, t) => a + (b - a) * t;
        return lerp(lerp(lerp(d(x0, y0, z0), d(x0 + 1, y0, z0), tx), lerp(d(x0, y0 + 1, z0), d(x0 + 1, y0 + 1, z0), tx), ty), lerp(lerp(d(x0, y0, z0 + 1), d(x0 + 1, y0, z0 + 1), tx), lerp(d(x0, y0 + 1, z0 + 1), d(x0 + 1, y0 + 1, z0 + 1), tx), ty), tz);
    };
    ...
    */
    ...
    const budget = (p) => p === 'bright'
        ? o.brightBudget
        : p === 'interesting'
            ? o.dimBudget * interesting
            : o.dimBudget * (1 - interesting);
    const mixOf = (p) => mixes[p];
    const expectedCount = (v, p) => budget(p) * density[v];
    const starsInVoxel = (v, p) => {
        if (v < 0 || v >= count)
            return [];
    ...
    */
    function starAt(v, p, k) {
        ...
        const candidate = (seed) => {
            ...
            for (let t = 0; t < MAX_TRIES; t++) {
                ...
                if (prng.value() * limit <= densityAt(pos))
                    break;
            }
        ...
        if (p === 'bright')
            return candidate(hash32(voxelSeed, k));
        /*
        ...
        */
        ...
        let star = candidate(hash32(voxelSeed, k, 0));
        for (let c = 0; c < MAX_CANDIDATES; c++) {
            if (c > 0)
                star = candidate(hash32(voxelSeed, k, c));
            star.bestHI = bestHIOf(star);
            if (isInteresting(star.bestHI) === want)
                break;
        }
        return star;
    }
    let brightCache = null;
    const brightStars = () => {
        if (brightCache == null) {
            brightCache = [];
            for (let v = 0; v < count; v++) {
                for (const s of starsInVoxel(v, 'bright'))
                    brightCache.push(s);
            }
        }
        return brightCache;
    ...
    const voxelsNear = (p, radius) => {
        const r2 = radius * radius;
        const lo = (c, half, size) => Math.max(0, Math.floor((c - radius + half) / size));
        const hi = (c, half, size, n) => Math.min(n - 1, Math.floor((c + radius + half) / size));
        const out = [];
    ...
    const dimStarsNear = (p, radius) => {
        const out = [];
        for (const v of voxelsNear(p, radius)) {
            for (const s of starsInVoxel(v, 'interesting'))
                out.push(s);
            for (const s of starsInVoxel(v, 'boring'))
                out.push(s);
        }
        return out;
    };
    let interestingCache = null;
    const interestingStars = () => {
        if (interestingCache == null) {
            interestingCache = [];
            for (let v = 0; v < count; v++)
                for (const s of starsInVoxel(v, 'interesting'))
                    interestingCache.push(s);
        }
    ...
    const nebulaPass = () => {
        if (nebulaCache == null) {
            const budget = o.nebulaBudget >= 0
                ? o.nebulaBudget
                : Math.max(50, Math.floor(o.brightBudget * 0.15));
            nebulaCache = generateNebulae(new PRNG(hash32(o.seed, NEBULA_STREAM)), budget, sp);
        }
        return nebulaCache;
    };
    let shellCache = null;
    const shell = () => {
        if (shellCache == null) {
            shellCache = generateShell(new PRNG(hash32(o.seed, SHELL_STREAM)), galaxyOpts.distantGalaxies, galaxyOpts.distantStars, sp, nebulaPass().densityScale);
        }
        return shellCache;
    };
    const toStarData = (s, planets) => {
    ...
    const view = (v = {}) => {
        ...
        const stars = brightStars().map((s) => toStarData(s, planets));
        for (const s of interestingStars())
    ...
    return {
        ...
        star(id) {
            ...
            if (a == null || a.seed !== o.seed || a.voxel >= count)
                return null;
            return toStarData(starAt(a.voxel, a.population, a.n), false);
        },
    };
}
//# sourceMappingURL=voxel-galaxy.js.map