...
function initialVertices(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (preset != null)
            return polygonVertices(preset.build());
    }
    return ngon(6);
}
/**
 ...
 * ```
 */
export function footprint3d(config = {}) {
    ...
    const draw = () => {
        ...
        axes.setAttribute('d', `M${box.x} ${cy}H${box.x + box.size}M${cx} ${box.y}V${box.y + box.size}`);
        axes.setAttribute('stroke', w3dTheme.divider);
        axes.setAttribute('stroke-width', '1');
        poly.setAttribute('d', verts
            .map((v, i) => {
            const c = toPx(v);
        ...
        poly.setAttribute('stroke-width', String(Math.max(1.5, w3dTheme.strokeWidth)));
        while (handles.firstChild)
            handles.removeChild(handles.firstChild);
        verts.forEach((v, i) => {
            const c = toPx(v);
            handles.appendChild(svgCircle({
                cx: c.x,
                cy: c.y,
...
//# sourceMappingURL=footprint-field.js.map