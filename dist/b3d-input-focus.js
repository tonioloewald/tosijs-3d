...
export class B3dInputFocus extends Component {
    ...
    _checkInteract = () => {
        if (!this.inputMappedProvider || !this.playerEntity || !this.owner)
            return;
        ...
        if (this.focusedEntity === this.playerEntity) {
            ...
            if (!playerTarget)
                return;
            const playerPos = playerTarget.absolutePosition ??
                playerTarget.position;
            const enterDist = this.enterDistance;
            // Find all controllable entities with enterable=true
            const candidates = Array.from(this.querySelectorAll('*')).filter((el) => {
                if (!(el instanceof B3dControllable))
                    return false;
                if (el === this.playerEntity)
                    return false;
                return el.enterable === true;
            });
            let closest = null;
    ...
    enterVehicle(vehicle) {
        ...
        if (playerTarget) {
            ;
            playerTarget.setEnabled?.(false);
        }
        // Focus on the vehicle
        this.focusEntity(vehicle);
    }
    exitVehicle() {
        if (!this.playerEntity || !this.focusedEntity)
            return;
        ...
        const exitPos = vehicleTarget
            ? (vehicleTarget.absolutePosition ??
                vehicleTarget.position).clone()
        ...
        if (playerTarget) {
            ;
            playerTarget.setEnabled?.(true);
            playerTarget.position.copyFrom(exitPos);
        }
