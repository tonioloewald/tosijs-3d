...
export function makeProbeScratch() {
    return {
        candidates: [],
...
const defaultFilter = (m) => m.isPickable !== false && m.isEnabled() && m.getTotalVertices() > 0;
/**
 ...
 * never allocates.
 */
export function readSurroundings(scene, feet, surr, options = {}) {
    ...
    */
    ...
    for (const m of scene.meshes) {
        ...
        if (BABYLON.Vector3.Distance(sphere.centerWorld, feet) <=
...
export class SurroundingsProbe {
    surroundings;
    hz;
    _scratch = makeProbeScratch();
    _since = Infinity;
    _elapsed = 0;
    constructor(surroundings, hz = 10) {
        this.surroundings = surroundings;
        this.hz = hz;
    }
    /** Advance the clock and read if due. `dt` is seconds. */
    update(scene, feet, dt, options = {}) {
        this._since += dt;
        this._elapsed += dt;
        if (this._since < 1 / Math.max(0.1, this.hz))
            return false;
        this._since = 0;
...
//# sourceMappingURL=surroundings-probe.js.map